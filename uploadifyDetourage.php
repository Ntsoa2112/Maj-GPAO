...
if (!empty($_FILES)) {
    ...
    foreach ($_FILES as $iFile) {			
        ...
        $targetDir = str_replace('//','/',$targetPath) .  '/39/' .$client;
        $targetFile =  $targetDir. '/' .$iFile['name'];
        $cheminRelatif = $_REQUEST['folder'] . '/39/' .$client. '/' .$iFile['name'];
        ...
        print_r($c->insertConsigneDetourage( $client, htmlspecialchars($iFile['name'])));
		...
		//if !(check_file_uploaded_length($targetFile)) echo ('telechargement impossible :' + $targetFile);	
	   ...
	   /*if (!is_dir($votreDossier."/".str_replace("%20"," ",$couper[count($couper)-1])."/")) 
	   {
		...
		$myfile = fopen("flip_temp/temp/".replaceCaractereSpeciaux($couper[count($couper)-1]).".txt", "w") or die("Unable to open file!");
