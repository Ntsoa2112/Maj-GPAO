...
//ajout CMUC
    ...
    function updateCqLot()
    {
        ...
        $.ajax({
            type: "GET",
            url: "php/link.php?action=updateCqLot&qt=" + qt + "&err=" + err + "&stat=" + stat + "&id=" + id + "&lib=" + lib + "&nature=0&distinction=0&traitement=1&ldg=" + ldg + "&doss=" + doss + "&idpers=" + idpers + "&idNuo=" + idNuo + "&idEtat=&idPs=" + idPs + "&isInterial=FALSE&is_rejet=" + isRejet + "&idMotifRejet=" + idMotifRejet, async: false,
            success: function (msg) {
                if (msg != "")
                {
                    alert(msg);
