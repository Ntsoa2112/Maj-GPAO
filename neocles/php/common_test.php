...
*/	
	...
	function UpdateCqLot($qt, $err, $stat, $id, $lib, $com, $ldg, $doss, $id_pers, $id_nuo, $id_ps)
	{
		...
		function GetSujetsTickets($_jourDebut,$_jour_fin){
			...
			$sqlSujet = "SELECT distinct(sujet) from action";
			$sqlDate = "SELECT distinct(jour_debut_action) FROM action WHERE jour_debut_action BETWEEN '".$_jourDebut."' AND '".$_jour_fin."'";
			//echo$sqlDate;
		...
		function GetIntervenantTickets($_jourDebut,$_jour_fin,$_choix){
			...
			$sqlIntervenant = "SELECT distinct(nom_prenom) from intervenant";
			$sqlDate = "SELECT distinct(jour_debut_action) FROM action WHERE jour_debut_action BETWEEN '".$_jourDebut."' AND '".$_jour_fin."'";
	  ...
	  function RechercheMaha($_date , $_pole , $_client , $_intervenant , $_ticket){
			...
			if($res == null || $res == false){
				...
				while($arr = $res->fetch()){
					...
					$str .= "<td id = date_arrivee".$arr['id'].">".$arr['date_arrivee']."</td>";
					$str .= "<td id = heure_arrivee".$arr['id'].">".$arr['heure_arrivee']."</td>";
					$str .= "<td id = client".$arr['id'].">".$arr['client']."</td>";
					$str .= "<td id = demandeur".$arr['id'].">".$arr['demandeur']."</td>";
					$str .= "<td id = heure_creation_ticket".$arr['id'].">".$arr['heure_creation_ticket']."</td>";
					$str .= "<td id = numero_ticket".$arr['id'].">".$arr['numero_ticket']."</td>";
					$str .= "<td id = pole_actuel".$arr['id'].">".$arr['pole_actuel']."</td>";
					$str .= "<td id = intervenant".$arr['id'].">".$arr['intervenant']."</td>";
					$str .= "<td COLSPAN = 3 id = commentaire".$arr['id'].">".$arr['commentaire']."</td>";
	  ...
	  function RechercheMahaSup($_date , $_etat , $_client , $_intervenant , $_ticket){
			...
			if($res == null || $res == false){
				...
				while($arr = $res->fetch()){
					...
					$str .= "<td id = heure_arrivee".$arr['id'].">".$arr['heure_arrivee']."</td>";
					$str .= "<td id = client".$arr['id'].">".$arr['client']."</td>";
					$str .= "<td id = origine".$arr['id'].">".$arr['origine']."</td>";
					$str .= "<td id = heure_creation_ticket".$arr['id'].">".$arr['heure_creation_ticket']."</td>";
					$str .= "<td id = numero_ticket".$arr['id'].">".$arr['numero_ticket']."</td>";
					$str .= "<td id = etat".$arr['id'].">".$arr['etat']."</td>";
					$str .= "<td id = intervenant".$arr['id'].">".$arr['intervenant']."</td>";
					$str .= "<td COLSPAN = 3 id = commentaire".$arr['id'].">".$arr['commentaire']."</td>";
					$str .= "<td><button id = '".$arr['id']."' class = 'button'>Modifier</button></td>";
		...
		function InsertHelpdesk($date_arrivee_php , $heure_arrivee_php , $client_php , $demandeur_php , $numero_ticket_php, $heure_creation_ticket_php, $pole_actuel_php, $intervenant_php, $commentaire_php){
			...
			{	  // REUSSITE DANS DE L'inscription
				...
				$requete = "SELECT id FROM  historique WHERE 
							date_arrivee = $date_arrivee_sql,
							heure_arrivee = $heure_arrivee_sql,
							client = $client_sql,
							demandeur = $demandeur_sql,
							heure_creation_ticket = $numero_ticket_sql,
							numero_ticket = $heure_creation_ticket_sql,
				...
				if($res != null && $res != false){
					...
					while($arr = $res->fetch()){
						...
						EcrireHistorique("insertion", $id_sql, "HELPDESK", $date_arrivee_sql);
						break;
					}
				}
		...
		function UpdateHelpdesk($id , $date_arrivee_php , $heure_arrivee_php , $client_php , $demandeur_php , $heure_creation_ticket_php, $numero_ticket_php, $pole_actuel_php, $intervenant_php, $commentaire_php){
			...
			{	  // REUSSITE DANS DE L'inscription
				EcrireHistorique("modification", $id_sql, "HELPDESK", $date_arrivee_sql);
				return "success";
			}	
		}
		...
		function DeleteHelpdesk($id){
			...
			{	  // REUSSITE DANS DE L'inscription
				EcrireHistorique("suppression", $this->cnx->quote($id), "HELPDESK", $date_arrivee_sql);
				return "success";
			}	
			...
			{	  // REUSSITE DANS DE L'inscription
				...
				if($res != null && $res != false){
					...
					while($arr = $res->fetch()){
						...
						EcrireHistorique("insertion", $id_sql, "SUPERVISION", $date_arrivee_sql);
						break;
					}
				}
		...
		function UpdateSupervision($id , $date_arrivee_php , $heure_arrivee_php , $client_php , $origine_php , $heure_creation_ticket_php, $numero_ticket_php, $etat_php, $intervenant_php, $commentaire_php){
			...
			{	  // REUSSITE DANS DE L'inscription
				...
				EcrireHistorique("modification", $id_sql, "SUPERVISION", $date_arrivee_sql);
		...
		function DeleteSupervision($id){
			...
			{	  // REUSSITE DANS DE L'inscription
				...
				EcrireHistorique("suppression", $this->cnx->quote($id), "SUPERVISION", $date_arrivee_sql);
				return "success";
			}	
		...
		function EcrireHistorique($action, $ligne, $type, $date){

			$matricul = $_SESSION['id'];
			$matricul = $this->cnx->quote($matricul);
			
			$type = $this->cnx->quote($type);
			$action = $this->cnx->quote($action);
			
			$requete = "INSERT INTO historique (matricul,action,id_ligne,type,date_creation) 
						VALUES ($matricul, $action, $ligne, $type, $date)";		
