...
<script language="javascript" type="text/javascript" charset="utf-8">
    ...
    $("#validNP").click(function () {
        ...
        $("#listePointage").hide();
        getLstPt(dPt, dateFin, mt, tp, dep);
        $("#listePointage").show('slow');
    });

    function getLstPt(dt, dateFin, mt, tp, dep)
    {
        //alert("php/link.php?action=getLstPt&dt="+dt+"&mt="+mt+"&tp="+tp+"&dep="+dep);
        var response = "";
        $.ajax({
            type: "GET",
            url: "php/link.php?action=getLstPt&dt=" + dt + "&dtfin=" + dateFin + "&mt=" + mt + "&tp=" + tp + "&dep=" + dep+"&ch=.", async: false,
