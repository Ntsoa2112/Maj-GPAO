...
//fonction utilisé pour verifier si c'est existant dans la base ou pas
  ...
  FROM almerys_category_daily where id_lotclient=$ldg  and date_deb='$datetoday' group by id_daily,id_lotclient, id_cq, id_categorie, date_deb";
    ...
    function InsertSLocalisationBureau($ipd_pers, $quartier, $temps, $NoteA, $NoteB, $NoteC, $NoteD, $NoteE, $NoteF, $InfoA, $InfoB, $InfoC, $InfoD, $InfoE, $InfoF) {
        if ($this->checkMatricule("enquete_Localisation_Bureau")) {
            return "Vous ête déjà dans la base de données!";
        } else {
    ...
    //fonction utilsé pour l'insertion du catégorie des poles pour le jour
    ...
    //Verifie si le matricule existe déjà dans la base de données
...
//        $tmpStr.="</table>";
    ...
    //fonction pour la mise à jour des données néocles table tb_neocles_ldt
   ...
   SET  intervenant='$interv', ticket='$ticket', interlocuteur='$interloc', type='$type', comportement='$comp', 
    ...
    function getOneLdt($id) {
        // recherche
        $sql = "SELECT p_ldt.id_ldt, p_ldt.id_dossier, p_ldt.date_deb_ldt,p_ldt.date_fin_ldt, r_personnel.appelation, r_personnel.matricule, p_dossier.num_dossier,p_etape.libelle as lib,p_etape.id_etape as etape, p_ldt.commentaire, p_type_ldt.libelle as type, p_ldt.h_deb, p_ldt.h_fin, p_ldt.quantite, p_ldt.nbre_erreur, p_etat.libelle as statu, p_ldt.machine ,p_ldt.id_ldt_neocles,tb_neocles_ldt.intervenant,tb_neocles_ldt. beneficiaire,tb_neocles_ldt. ticket,tb_neocles_ldt. action,tb_neocles_ldt. information,tb_neocles_ldt. 
       pilote,tb_neocles_ldt. interlocuteur,tb_neocles_ldt. type,tb_neocles_ldt. comportement,tb_neocles_ldt. societe,tb_neocles_ldt. description FROM p_ldt
			...
			LEFT JOIN p_etat ON p_ldt.id_etat=p_etat.id_etat
			LEFT JOIN p_type_ldt on p_type_ldt.id_type_ldt = p_ldt.id_type_ldt
			LEFT JOIN tb_neocles_ldt on tb_neocles_ldt.id_ldt_neocles = p_ldt.id_ldt_neocles
    ...
    function getNeoclesLdt($doss, $mat, $lstPeriode, $stat, $orderby, $dep, $etape, $typen, $comp) {
        ...
        if ($dep != "")
            $filtre .= " AND id_departement = $dep";
        if ($doss != "")
            $filtre .= " AND p_ldt.id_dossier = $doss";
        if ($mat != "")
        ...
        $sql = "SELECT p_ldt.id_ldt, p_ldt.id_lot, p_ldt.id_dossier, p_ldt.machine, p_ldt.date_deb_ldt,p_ldt.date_fin_ldt, r_personnel.appelation, r_personnel.matricule, p_dossier.num_dossier,p_etape.libelle as lib, p_ldt.commentaire, p_type_ldt.libelle as type, p_ldt.h_deb, p_ldt.h_fin, p_ldt.quantite, p_ldt.nbre_erreur, p_etat.libelle as statu, p_lot.libelle as liblot, p_lot_client.libelle as liblotclient,
		DATE_PART('epoch', ('2011-12-29 '||p_ldt.h_fin)::timestamp - ('2011-12-29 '||p_ldt.h_deb)::timestamp ) as duree , p_lot_client.libelle as ldg,tb_neocles_ldt.id_ldt_neocles,tb_neocles_ldt.intervenant as intervenant,tb_neocles_ldt.beneficiaire as beneficiaire,tb_neocles_ldt.ticket as ticket,tb_neocles_ldt.action as action,tb_neocles_ldt.information as information,tb_neocles_ldt.interlocuteur as interloc,tb_neocles_ldt.type,tb_neocles_ldt.comportement,tb_neocles_ldt.societe as soc,tb_neocles_ldt.description FROM p_ldt
			LEFT JOIN p_dossier ON p_ldt.id_dossier=p_dossier.id_dossier
			LEFT JOIN p_lot ON p_ldt.id_lot=p_lot.id_lot
			LEFT JOIN p_lot_client ON p_ldt.id_lotclient=p_lot_client.id_lotclient
			LEFT JOIN p_lien_oper_dossier ON p_ldt.id_etape=p_lien_oper_dossier.id_lien
			...
			LEFT JOIN p_type_ldt on p_type_ldt.id_type_ldt = p_ldt.id_type_ldt
			LEFT JOIN r_personnel ON r_personnel.id_pers=p_ldt.id_pers
                        LEFT JOIN tb_neocles_ldt ON  p_ldt.id_ldt_neocles= tb_neocles_ldt.id_ldt_neocles

			WHERE tb_neocles_ldt.id_ldt_neocles<>0 ";
        $sql .= $filtre;
        ...
        foreach ($rs as $row) {
            ...
            $str .= "<td><div class=\"ldt_nb\">" . $row['intervenant'] . "</div></td>";
            $str .= "<td OnClick=updateLdtForm('" . $row['id_ldt'] . "') class='edit'></td>";
            //pour la liste des intervenants et durée
            if ($lastintervenant != $row['intervenant'] || $lastdate != $row['date_deb_ldt'] || $lastdate!="") {
        ...
        $str.="</table> <hr/>";
        $str.="<table><thead><tr><td>Date</td><td>Intervenant</td><td>Nombre d'appel</td><td>Dur&#233;e</td></tr>";
        foreach ($listIntervenant as $key => $value) {
            $tk = explode("_", $value);
            $str.="<tr><td>$tk[2]</td><td>$key</td><td>$tk[1]</td><td>$tk[0]</td></tr>";
        }
        $str.= "</thead></table>";
        return $str;
    }
...
//}
  ...
  ORDER BY almerys_p_lot.id_pers";
    ...
    /*
      on a supprimé la saisie des montant, et remplacé par le click d'un seul bouton pour les niveau notamment FAV_A,  FAV_B,  FAV_C, NON_FAV
...
//fonction peremttant de lister les categorie existant dans la table categorie
  ...
  FROM almerys_user WHERE 1=1" . $filtre . "order by sat ASC";
    ...
    //fonction permettant de lister les libellé de retour existant dans la table almerys_user
...
//                    return "false be!!!";
            ...
            //requete pour verifier la catégorie du pole ajouté
