...
//            return "aucun.htm";
   ...
   SET  intervenant='$interv', ticket='$ticket', interlocuteur='$interloc', type='$type', comportement='$comp', 
    ...
    function getIdLotAlmerys($id) {
        $sql = "select libelle, qte, id_lotclient, id_pers, num_nuo, num_ps, is_interial, is_rejet, id_motif_rejet,id_tache,id_distinction from almerys_p_lot where id_lot = " . $id;
        $rs = $this->cnx->query($sql);

        foreach ($rs as $row) {
            return $row['libelle'] . '$' . $row['qte'] . '$' . $row['id_lotclient'] . '$' . $row['id_pers'] . '$' . $row['num_nuo'] . '$' . $row['num_ps'] . '$ ' . $row['is_interial'] . '$' . $row['is_rejet'] . '$' . $row['id_motif_rejet'] . "$" . $row['id_tache']."$".$row['id_distinction'];
        }
        return '';
    }
...
//fonction permettant de lister les type existant pour chaque pole
    ...
    //fonction permettant de valider les controle qualité des saisie
    function UpdateCqLot($qt, $err, $stat, $id, $lib, $nature,$distinction, $ldg, $doss, $id_pers, $id_nuo, $id_etat, $id_ps, $is_interial, $is_rejet, $idMotifRejet) {
...
//                        else
                ...
                return 'Possible Doublon!, attention!.'.$sql;
            } else {
...
/////////////////////////////////////////////////////////////fin ajout comptage cq////////////////////////////////////////
    ...
    function pointage($id_pers, $pt, $h, $d) {
        ...
        $cc = new Cnxx();
        $arrayAllowedMatricul = array('177' => '', '1' => '', '551' => '', '487' => '487');
...
//        return $fName;
    ...
    function isSuperAdmin($matricule){
	 ...
	 //aza fafana le 868 ry volol ah
	//nga akanonona numéro mahazo izy ty e :p
              $admin = [1,177,551,1020,868,70,6,730,552,1084,949,421,709,487, 103];
