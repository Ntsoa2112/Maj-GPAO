...
class Cnx extends PDO {
    ...
    //focntion utilsé par enquete bien-etre pour l'insertion de donnée
    function insertSurvey($ipd_pers, $reveil, $coucher, $trajet, $distance, $transport) {

        $sql = "INSERT INTO enquete_bien_etre(id,id_pers, reveil, coucher, duree_trajet, distance, transport) VALUES(DEFAULT," . $ipd_pers . ",'" . $reveil . "','" . $coucher . "','" . $trajet . "','" . $distance . "','" . $transport . "')";
        $this->cnx->exec($sql);
    }

//focntion utilisé par la requete bien-etre pour la lecture des données
    function getListSurvey($ipd_pers, $reveil, $coucher, $trajet, $distance, $transport) {
        $sql = "SELECT id,id_pers,reveil,coucher,duree_trajet,distance,transport from enquete_bien_etre where id_pers = $ipd_pers";

        $rs = $this->cnx->query($sql);
        $str = "<table>";

        foreach ($rs as $row) {
            $str .= "<tr><td>" . $ipd_pers . "</td><td>" . $reveil . "</td><td>" . $coucher . "</td><td>" . $trajet . "</td><td>" . $distance . "</td><td>" . $transport . "</td>";
        }
        $str .= "</table>";
        return $str;
    }
    ...
    function checkMatricule() {
        ...
        $sqlPointage = "select count(*) from enquete_bien_etre where id_pers = " . $_SESSION['id'];
...
//		if ($all == 'ALL')
//                    //requete pour la liste des personne présent et qui n'ont pas encore fait l'enquete
//		$sqlPointage = "select id_pers from r_personnel where actif=true and id_pers not in (select id_pers from enquete_bien_etre order by id_pers) order by id_pers";
//		$rs1=$this->cnx->query($sqlPointage);
//		$str = '';
//		while($arr1 = $rs1->fetch())
//		{
//			if ($all == 'ALL') $str .= ' - '.$arr1['id_pers'] ;
//			else return true;				
//		} 
//		return $str;
    }
    ...
    function enqueteDejaFait($all) {
        $sqlPointage = "select * from enquete_bien_etre where id_pers = " . $_SESSION['id'];
        if ($all == 'ALL')
            $sqlPointage = "select id_pers from r_personnel where actif=true and id_pers not in (select id_pers from enquete_bien_etre order by id_pers) order by id_pers";
    ...
    function suiviHeureOP($intervalle, $dossier, $departement, $matricule) {
        ...
        if ($departement != "") {
            ...
            if (count($tk) > 1)
                $filtreIntervaleTemps = " AND p_ldt.date_deb_ldt >=  '" . $tk[0] . "' AND p_ldt.date_deb_ldt <= '" . $tk[1] . "'";
            $filtreDepartement = " LEFT JOIN r_personnel ON r_personnel.id_pers=p_ldt.id_pers
			WHERE r_personnel.id_departement = $departement $filtreIntervaleTemps";
            $filtreIntervaleTemps = "";
        }
        $sqlDoss = "SELECT id_pers FROM r_personnel " . $filtreDepartement . " order by id_pers";

        $sql = "select SUM(DATE_PART('epoch', ('2011-12-29 '||h_fin)::timestamp - ('2011-12-29 '||h_deb)::timestamp )) as somme, p_ldt.id_pers from p_ldt $filtreDepartement $filtreIntervaleTemps  group by p_ldt.id_pers order by p_ldt.id_pers desc";

        if ($matricule != "") {
            $sql = "select SUM(DATE_PART('epoch', ('2011-12-29 '||h_fin)::timestamp - ('2011-12-29 '||h_deb)::timestamp )) as somme, p_ldt.date_deb_ldt as id_pers from p_ldt $filtreIntervaleTemps AND id_pers = $matricule group by p_ldt.date_deb_ldt order by p_ldt.date_deb_ldt ";
        }
	...
	function TempsParDossierEtape($dossier, $etape, $lstPeriode, $matricule, $statut, $orderby, $dep)
	{
		...
		//return $lstPeriode;
		if ($dep != "") 		$filtre .= " AND r_personnel.id_departement = $dep";
	...
	function getLdt($dossier, $etape, $lstPeriode, $matricule, $statut, $orderby, $dep)
	{
		...
		if ($dep != "") 		$filtre .= " AND id_departement = $dep";		
		if ($dossier != "") 	$filtre .= " AND p_ldt.id_dossier = $dossier";
	...
	LEFT JOIN almerys_motif_rejet ON almerys_motif_rejet.id= almerys_p_lot.id_motif_rejet 
		...
		$POLE = $boolInterial?"SPECIALITE":"POLE";
			...
			LEFT JOIN p_lien_oper_dossier ON almerys_p_lot.id_etape=p_lien_oper_dossier.id_lien 
			LEFT JOIN p_etape ON p_lien_oper_dossier.id_oper=p_etape.id_etape
			where almerys_p_lot.id_etape <> 943 and date_deb = '$prio' 
		...
		foreach($rs as $row)
		{
			...
			if($boolInterial)			
			{
				if ($montant >= 350) $niveau = 'NIVEAU_A';
				if ($montant < 350) $niveau = 'NIVEAU_B';
			}
			else
			{
				...
				switch ($row['id_lotclient'])
				{
					...
					case 2386: //OPTIQUE
						...
						if ($montant <= 300) $niveau = 'NIVEAU_C';
						break;
