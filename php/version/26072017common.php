...
//            return "aucun.htm";
   ...
   SET  intervenant='$interv', ticket='$ticket', interlocuteur='$interloc', type='$type', comportement='$comp', 
    ...
    function getLdt($dossier, $etape, $lstPeriode, $matricule, $statut, $orderby, $dep, $deb, $fin, $sous_spe,$sous_spe2) {
...
$dep_test=date('Ymd', strtotime($date));
        if ($dossier != "")
            $filtre_almerys .= " AND p_ldt.id_dossier = $dossier";
        if ($deb != "")
            $filtre_almerys .= " AND p_ldt.date_deb_ldt = '$dep_test' ";     
        if ($dep != "")
        {
            $filtre_almerys .= " AND r_personnel.matricule = '$matricule'";
        }
...
LEFT JOIN almerys_p_lot_new ON p_ldt.id_lot = almerys_p_lot_new.id_lot
    ...
    foreach ($rs as $row) 
         {
                ...
                if ($row['id_lot'] != 0) {
                    $str .= "<td><div class=\"ldtlib\">" . $row['id_lot'] . "</div></td>";
...
/////////////////////////////////////////////////////////////fin ajout comptage cq////////////////////////////////////////
    ...
    function getLstSousSpecialite($idDossier) {
        $sql = "SELECT id_ss_spe, id_dossier, id_lotclient, libelle FROM almerys_ss_spe WHERE id_dossier=$idDossier";
        $rs = $this->cnx->query($sql);
        $str = "<option></option>";
        //return $sql;
        while ($arr = $rs->fetch()) {
...
//        return $fName;
    ...
    function isSuperAdmin($matricule) {
        ...
        $admin = array('487');
	  // $admin = [1, 177, 551, 1020, 70, 6, 730, 552, 1084, 949, 421, 709, 725, 103,1241];
