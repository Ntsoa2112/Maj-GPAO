  ...
  //foc ntion pour l'enquete medaille travail 
    function insertSondage($ipd_pers, $anneetrvail, $decormedail) {
        if ($this->checkMatricule()) {
            ...
            $sql = "INSERT INTO enquete_medaille(id_pers,valeurquestion1, valeurquestion2) VALUES(" . $ipd_pers . ",'" . $anneetrvail . "','" . $decormedail . "')";
            $this->cnx->exec($sql);
    ...
    function enquete($score, $priorite, $Soarano, $Ankorondrano, $Alarobia, $Andraharo) {
        ...
        $sql = "select SUM(DATE_PART('epoch', ('2011-12-29 '||h_fin)::timestamp - ('2011-12-29 '||h_deb)::timestamp )) as somme,  sum(to_number('0'||quantite,'99999')) as qte, p_ldt.id_pers, id_type_ldt, r_personnel.appelation,(select  SUM(nb_rejet) +SUM(nb_valide)+SUM(nb_tempo)from almerys_comptage  WHERE almerys_comptage.id_pers=p_ldt.id_pers ".$filtredat.") AS total
		FROM p_ldt 
...
//        $tmpStr.="</table>";
	...
	function getInfo($idPers, $dossier, $numClass) {
	 ...
	 function getEtat($idPers, $numClass) {
        ...
        foreach ($rs as $row) {
			...
			else return  "<tr class='NIVEAU_B $strRet<td>Attente de travail</td> <td> </td> <td>".$row['h_fin']."</td><td> </td></tr>";
	...
	if (!isset($_SESSION['pseudo']))
		...
		$rs = $this->cnx->exec("INSERT INTO p_historique_suppression(id_pers, id_dossier, historique, current_table, ip, date_suppression, heure_suppression)  VALUES (".$_SESSION['id'].", $idDossier, '.$jsonRow.', '$currentTable', '".$_SERVER['REMOTE_ADDR']."', substr(now()|| ' ', 0, 5)||substr(now()|| ' ', 6, 2)||substr(now()|| ' ', 9, 2), substr(now()|| ' ', 12, 8));");
