...
//fonction enquete par defaut 
    ...
    function checkMatricule() {
        ...
        $sqlPointage = "select count(*) as insertion  from enquete_medaille where id_pers = " . $_SESSION['id']; //edti by Modestio:
        // return $sqlPointage;
    ...
    function enqueteDejaFait($all) {
        $sqlPointage = "select * from enquete_medaille where id_pers = " . $_SESSION['id'];//enquete_smq
        if ($all == 'ALL')
            $sqlPointage = "select id_pers from r_personnel where actif=true and id_pers not in (select id_pers from enquete_medaille order by id_pers) order by id_pers";
...
//                $vitesse_nette = ($qte != 0) ? number_format($qte / ($prod / 3600), 2) : '';
                ...
                $arrayVitBrut[$arr['id_pers']] = $arr['total'] . "_" . $vitesse_brute . "_" . $vitesse_nette;
                ...
                $tmpStr .= "<td class=' " . $cl . "_1'><h4  class='title4'>" . $arr['total'] . "</h4></td>"; //$qte by $arr['total']
    ...
    function getLdt($dossier, $etape, $lstPeriode, $matricule, $statut, $orderby, $dep) {
        ...
        if ($lstPeriode != "") {
            ...
            if (count($tk) > 1) {
                ...
                $filtredat = " AND date_deb='$lstPeriode'";
        }
        ...
        $sql = "SELECT p_ldt.id_ldt, p_ldt.id_lot, p_ldt.machine, p_ldt.date_deb_ldt,p_ldt.date_fin_ldt, r_personnel.appelation, r_personnel.matricule, p_dossier.num_dossier,p_etape.libelle as lib, p_ldt.commentaire, p_type_ldt.libelle as type, p_ldt.h_deb, p_ldt.h_fin, p_ldt.quantite,(select  SUM(nb_rejet) +SUM(nb_valide)+SUM(nb_tempo)from almerys_comptage  WHERE almerys_comptage.id_pers=p_ldt.id_pers" . $filtredat . ") AS total, p_ldt.nbre_erreur, p_etat.libelle as statu, p_lot.libelle as liblot, p_lot_client.libelle as liblotclient,
...
//                if ($row['quantite'] != '' && $row['quantite'] != 0)
            ...
            $str .= "<td><div class=\"ldt_nb\">" . $row['total'] . "</div></td>";
    ...
    function getLot($dossier, $lotClient, $matricule, $statut, $etape, $prio, $name, $ordre) {
        //  return $dossier." ".$lotClient." ".$matricule." ".$statut." ".$etape." ".$prio." ".$name." ".$ordre
        $filtre = "";
        if ($dossier != "") {
...
//}
  ...
  ORDER BY almerys_p_lot.id_pers";
     ...
     */
    function getLotAlmerys($dossier, $lotClient, $matricule, $statut, $etape, $prio, $last, $name, $ordre, $is_interial) {
        $filtre = "";
        if ($dossier != "")
            $filtre .= " AND p_dossier.id_dossier = $dossier";
