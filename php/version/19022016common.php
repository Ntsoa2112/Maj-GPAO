...
//        $tmpStr.="</table>";
    ...
    function getLstConsigne($idDossier, $idEtape) {
        $sql = "SELECT p_consigne.id, p_dossier.num_dossier,p_etape.libelle, p_consigne.url, p_consigne.id_pers, p_consigne.date_consigne from p_consigne
        ...
        foreach ($rs as $row) {
            ...
            if ($_SESSION['id_droit'] == 1 && $idEtape == "") return "";

            $etape = ($idEtape=="")?"":($idEtape.'/');
            $path = 'consignes/'.$idDossier.'/'.$idEtape.$row['url'];
...
//edit by vololona add col nature
  ...
  WHERE almerys_p_lot.id_etape <> 943
    AND date_deb        = '" . str_replace("/", "", $prio) . "'
    AND p_etape.libelle IN ('NRRG', 'ES','OK')
    AND is_interial=$is_interial
  GROUP BY
        almerys_p_lot.id_pers
      , p_lot_client.libelle
  ...
  ORDER BY almerys_p_lot.id_pers";
            ...
            while ($arrDoss = $rss->fetch()) {
                ...
                if ($ii == 3)
                    $ii = 0;
                $total = 0;
                $tk = $arrayIdPersCountSaisie[$arrDoss['op']];
                $erreur = $arrDoss['nb'] - $tk[0];
                $rate = ($erreur / $arrDoss['nb']) * 100;
                $str .= "<tr class = 'classe" . $ii . "'><td>" . $arrDoss['sat'] . "</td><td>" . $arrDoss['op'] . '</td><td>' . $arrDoss['lib'] . '</td><td>' . $erreur . '</td><td>' . $arrDoss['nb'] . '</td><td>' . number_format($rate, 2) . '</td></tr>';
                $ii++;
            }
...
/////////////////////////////////////////////////////////////fin ajout comptage cq////////////////////////////////////////
	...
	function identification($user, $pass) {
		...
		else
		{
			...
			$sql = "select ldap_name, nom, prenom , id_pers, id_droit from r_personnel where ldap_name = '$user'";
