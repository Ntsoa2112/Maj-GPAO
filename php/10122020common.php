...
//            return "aucun.htm";
   ...
   function getLdt($dossier, $etape, $lstPeriode, $matricule, $statut, $orderby,$lot_libelle, $dep, $deb, $fin, $spe, $sous_spe, $sous_spe2) {
        ...
        $filtre = "";
        $filtre_almerys= "";
        $poleAlm = array(712, 577, 579, 578, 580, 29, 582, 663, 701, 702, 703, 704, 41,35);
        if ($lstPeriode != "") {
            $filtre = " AND p_ldt.date_deb_ldt = '" . $lstPeriode . "'";
	...
	p_lot_client.libelle as ldg,p_ldt.ver as version_appli,p_lot.qte as quantite_lot,almerys_ss_spe.libelle  as sous_spe FROM p_ldt
			LEFT JOIN p_dossier ON p_ldt.id_dossier=p_dossier.id_dossier
  ...
  if($dossier==712 || $dossier==577 || $dossier==579 || $dossier==578 || $dossier==580 || $dossier==29 || $dossier==582 || $dossier==663 ||  $dossier==721 || $dossier==723 || $dossier==724 || $dossier==35)
          {   
        if ($dossier != "")
...
WHERE 1=1 ";
 ...
 $sql .= $filtre_almerys;
        ...
        $ii = 0;
         ...
         if($dossier==712 || $dossier==577 || $dossier==579 || $dossier==578 || $dossier==580 || $dossier==29 || $dossier==582 || $dossier==663 ||  $dossier==721 || $dossier==723 || $dossier==724)
        {
...
//              $str .= '<tr  class = "th"><td><b>Total:</b></td><td></td><td></td></td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td><b>' . number_format($hTotalLdt / 3600, 2) . '</b></td><td><b>' . $qteTotal . '</b></td><td><b>' . $totalVitess . '</b></td><td></td></tr>';
    ...
    function getLot($dossier, $lotClient, $matricule, $statut, $etape, $prio,$checkonelot, $name, $ordre, $sous_spe) {
        ...
        ,p_lot.id_pers, p_lot.priority, p_lot.id_pers, p_lot.duree, p_lot.qte, p_lot.qte as qte_commande, p_lot.nbre_erreur,almerys_ss_spe.libelle as sous_spe FROM p_lot
        ...
        if ($dossier == 712)
            $str .= "<th  class='th' width='20%' id='sous_spe' class='filter'>Sous Specialite</th>";
        if($dossier == 837)
        {
...
/////////////////////////////////////////////////////////////fin ajout comptage cq////////////////////////////////////////
   ...
   //    return $this->loginOK($user, $pass);
    ...
    function pointage($id_pers, $pt, $h, $d) {
        ...
        $cc = new Cnxx();
        $arrayAllowedMatricul = array('177' => '', '551' => '');
    ...
    function lstPointage($datePt, $dateFin, $matriPt, $typePoint, $departement, $ch) {

        //prendre les subordonnees
        //$subordonnee = $this->getSubordonee(1084);
...
//            creation du fichier
        ...
        if ($typePoint != "") {
            ...
            if (strstr($typePoint, 'IN_ARO'))
                $qType = " AND source IN ('IN_ARO', 'IN_ARO1', 'IN-1', 'IN-2', 'IN-3', 'IN-4', 'IN')";
            if (strstr($typePoint, 'OUT_ARO'))
                $qType = " AND source IN ('OUT_ARO', 'OUT_ARO1', 'OUT-1', 'OUT-2', 'OUT-3', 'OUT-4', 'OUT')";
        }
        else {
            $qType = " AND source IN ('IN', 'OUT','IN_ARO', 'OUT_ARO','IN_ARO1', 'OUT_ARO1', 'IN_RDJ', 'OUT_RDJ', 'IN-1', 'IN-2', 'IN-3', 'IN-4', 'IN', 'OUT-1', 'OUT-2', 'OUT-3', 'OUT-4' , 'OUT') ";
        }

        $fName .= "_.csv";

        $fw = fopen($fName, 'w');
        //$fw = fopen("./CSV/NAMEEEEEE.csv", 'w');
