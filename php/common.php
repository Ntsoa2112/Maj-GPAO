...
class Cnx extends PDO {
    ...
    const DEFAULT_DNSGPAO = 'pgsql:host=db1.easytech.mg port=5432;dbname=easygpao';
    ...
    public function Cnx() {
        ...
        $this->cnxgpao = new PDO(
                self::DEFAULT_DNSGPAO, self::DEFAULT_SQL_USER, self::DEFAULT_SQL_PASS
        );
    }
...
//fonction utilis� pour verifier si c'est existant dans la base ou pas
  ...
  FROM almerys_category_daily where id_lotclient=$ldg  and date_deb='$datetoday' group by id_daily,id_lotclient, id_cq, id_categorie, date_deb";
    ...
    //fonction pour l'enquete medaille travail 
    ...
    function updateMac($id) {
        ...
        $WbemServices = $WbemLocator->ConnectServer($pc, 'root\\cimv2', 'mirah', 'Ra123456$1985$*', "MS_409", "ntlmdomain:" . $domaine);
    ...
    function suiviHeureOP($intervalle, $dossier, $departement, $matricule) {
        ...
        $fName = $folder . '/RappOp_' . $dossier . '_' . str_replace(",", "_", $intervalle) . '.csv';


        if (!$isOP)
            $fw = fopen($fName, 'w');
        $filtre = "";
        $filtreIntervaleTemps = "";
        $filtredat = "";
        if (count($tk) > 1) {
            //  $filtreIntervaleTemps = " AND p_ldt.date_deb_ldt >=  '" . $tk[0] . "' AND p_ldt.date_deb_ldt <= '" . $tk[1] . "'";
            $filtreIntervaleTemps = " AND p_ldt.date_deb_ldt between '$tk[0]' AND '$tk[1]'";
        } else
            $filtreIntervaleTemps = " AND p_ldt.date_deb_ldt = '" . $intervalle . "'";
        ...
        $sql = "select SUM(DATE_PART('epoch', ('2011-12-29 '||p_ldt.h_fin)::timestamp - ('2011-12-29 '||p_ldt.h_deb)::timestamp )) as somme,  sum(to_number('0'||quantite,'99999')) as qte, p_ldt.id_pers, id_type_ldt,p_dossier.num_dossier as dossier, r_personnel.appelation,p_ldt.h_fin,p_ldt.h_deb
		FROM p_ldt 
		...
		<td class='vertical-text gray nopading '>FUNHENCE</td>
        ...
        $export = "MATRICULE\tVITESSE NETTE\tVITESSE BRUTE\tQTE\tPROD BRUTE"
                . "\tPROD NETTE\tHORS PROD\tPAUSE\tFORMATION\tATTENTE_DE_TRAVAIL"
                . "\tPAUSE_DEJEUNER\tPANNE_MACHINE\tPANNE_RESEAU\tDelegues_du_personnel"
                . "\tPAUSE_TOILETTE\tINSTALLATION\tFUNHENCE\tREUNION\tMAINTENANCE"
                . "\tPERMISSION\tEXERCICE_Sous_charge\tPROBLEME_APPLICATION\tREFECTION";
        ...
        while ($arr = $rsDoss->fetch()) {
            ...
            if ($lastIdPers <> $arr['id_pers']) {
                ...
                $export .= str_replace('0.00', "", $tempExport);
                $ii++;
                $nbOp++;
                $sommeQte += $qte;
                $sommeHeureProd += $prod;
            ...
            $arrayDossier[$arr['dossier']] = $heureprod_dossier . "_" . $horsprod_dossier; {
                ...
                $tempExport = $pers . " - " . $arr['appelation'] . "\t$vitesse_nette\t$vitesse_brute\t$qte"
                        . "\t" . sprintf("%01.2f", $total / 3600) . "\t" . sprintf("%01.2f", $prod / 3600)
                        . "\t" . sprintf("%01.2f", $horsProd / 3600) . "\t" . sprintf("%01.2f", $PAUSE / 3600)
                        . "\t" . sprintf("%01.2f", $FORMATION / 3600) . "\t" . sprintf("%01.2f", $ATTENTE_DE_TRAVAIL / 3600)
                        . "\t" . sprintf("%01.2f", $PAUSE_DEJEUNER / 3600) . "\t" . sprintf("%01.2f", $PANNE_MACHINE / 3600)
                        . "\t" . sprintf("%01.2f", $PANNE_INTERNET / 3600) . "\t" . sprintf("%01.2f", $PANNE_RESEAU / 3600)
                        . "\t" . sprintf("%01.2f", $Delegues_du_personnel / 3600) . "\t" . sprintf("%01.2f", $PAUSE_TOILETTE / 3600)
                        . "\t" . sprintf("%01.2f", $INSTALLATION / 3600) . "\t" . sprintf("%01.2f", $OSTIE / 3600)
                        . "\t" . sprintf("%01.2f", $REUNION / 3600) . "\t" . sprintf("%01.2f", $MAINTENANCE / 3600)
                        . "\t" . sprintf("%01.2f", $PERMISSION / 3600) . "\t" . sprintf("%01.2f", $EXERCICE_Sous_charge / 3600)
                        . "\t" . sprintf("%01.2f", $PROBLEME_APPLICATION / 3600) . "\t" . sprintf("%01.2f", $TEST_APPLICATION / 3600)
                        . "\t" . sprintf("%01.2f", $REFECTION / 3600) . "\n";
        ...
        $export .= "\n TOTAL\t$sommeQte\t" . sprintf("%01.2f", ($sommeHeure / 3600))
                . "\t" . sprintf("%01.2f", $sommeHeureProd / 3600) . "\t" . sprintf("%01.2f", $sommeHeureHorsProd / 3600) . "\n"
                . "AVG Pondere brut : " . sprintf("%01.2f", $totalMoyenne / 3600) . " nette :" . sprintf("%01.2f", $totalnette / 3600);
        ...
        $str .= "</tr>";
        $str .= "</table>";
        $str .= "<hr> <h4  class='title4 texte_bleu'>AVG Pondere brut : " . sprintf("%01.2f", $totalMoyenne / 3600) . " nette :" . sprintf("%01.2f", $totalnette / 3600) . " </h4>";
        if (!$isOP) {
            fwrite($fw, utf8_decode($export));
            fclose($fw);
        }
        $excel = "<table><tr><td><h4>Exporter: </h4></td><td><a href='./CSV/$fName' OnClick=deleteReport('" . $matricule . "')><input type='submit' value='' class='copy' title='Exporter au format CSV'/></a></td></tr></table>";
        return $excel . $str;
    }

    function suiviHeureOP1($intervalle, $dossier, $departement, $matricule) {
        set_time_limit(30000);
        $tk = explode(",", $intervalle);
        $filtreIntervaleTemps = " AND p_ldt.date_deb_ldt = '" . $intervalle . "'";
        $filtredat = "";
    ...
    function suiviHeureOP_($intervalle, $dossier, $departement) {
        ...
        while ($arr = $rsDoss->fetch()) {
            ...
            $str .= "<td>" . number_format($somme, 2, ',', '') . "</td>";
            $str .= "</tr>";
        }
    ...
    function getLstConsigne($idDossier, $idEtape) {
        ...
        foreach ($rs as $row) {
            ...
            $CheminFlip = "../consignes/" . $CheminFlip . "FB/" . $this->replaceCaractereSpeciaux($tempsPath[count($tempsPath) - 1]) . "/";
            //fin : Chemin flipbook
...
//            return "aucun.htm";
    ...
    function replaceCaractereSpeciaux($caractereSpeciaux) {
        $Caracte = array("#", "[", "]", "é", " ", "!", "(", ")");
        $equivalence = array("1", "2", "3", "e", "_", "4", "5", "6");
        for ($i = 0; $i < count($Caracte); $i++) {
            $caractereSpeciaux = str_replace($Caracte[$i], $equivalence[$i], $caractereSpeciaux);
        }

        if (strlen($caractereSpeciaux) > 138) {
            $caractereSpeciaux = substr($caractereSpeciaux, 0, 134) . ".pdf";
        }
    ...
    function getListConnected($dossier, $departement, $matr) {
        $currDate = date("Y/m/d");
        $batiment = 'IN';

        $sql = "SELECT distinct(id_util) from r_pointage where pdate='" . $currDate . "' order by id_util";
        if ($matr != "" && $matr != null)
            $sql = "SELECT distinct(id_util) from r_pointage where pdate='" . $currDate . "'  AND id_util=" . $matr . " order by id_util";
        ...
        if ($departement != "") {
            ...
            if ($matr != "" && $matr != null)
                $sql = "SELECT r_personnel.id_pers as id_util, appelation FROM r_personnel
				LEFT JOIN r_affectation ON r_affectation.id_pers = r_personnel.id_pers
				LEFT JOIN r_departement ON r_affectation.id_departement = r_departement.id  WHERE  r_departement.id = " . $departement . " AND actif = true AND r_personnel.id_pers=" . $matr . " order by r_personnel.id_pers";
        }
        ...
        if ($dossier != "" & $departement == "") {
            ...
            if ($matr != "" && $matr != null)
                $sql = "SELECT distinct(r_personnel.id_pers) as id_util, appelation FROM r_personnel
				LEFT JOIN p_affectation ON p_affectation.id_pers = r_personnel.id_pers
				WHERE actif = true AND p_affectation.id_dossier = " . $dossier . " AND r_personnel.id_pers=" . $matr . " order by r_personnel.id_pers";
        }
    ...
    function ToHeureDecimalAffiche($strTime) {
        $h = explode(":", $strTime);
        return number_format($h[0] + ($h[1] / 60), 2);
    }
    ...
    function hDiff($t1, $t2, $oper) {
        ...
        else
		{
			...
			if($h2<$h1)
				$temp=$h2+24;
			$ret = (($temp * 3600) + ($m2 * 60) + ($s2)) - (($h1 * 3600) + ($m1 * 60) + ($s1));
		}
        $h = floor($ret / 3600);
    ...
    function delConsigne($p_id_ldt) {
        ...
        $temps = str_replace("/var/www/html/$/", "../", $temps) . "/";
    ...
    function updateLot($lstId, $etat, $prio, $mat) {
        ...
        // $arrayAllowedMatricule = array('487' => '', '218' => '', '238' => '', '1048' => '', '233' => '', '62' => '');

        $sql .= $filtre . " where id_lot IN ($lstId)";
        /* if (array_key_exists($_SESSION['id'], $arrayAllowedMatricule) && $etat == "2")
          $sql = "update p_lot set  id_etat = $etat where id_lot IN ($lstId) "; */
        // return $sql;
   ...
   SET  intervenant='$interv', ticket='$ticket', interlocuteur='$interloc', type='$type', comportement='$comp', 
    ...
    function updateLdt($deb, $fin, $qt, $err, $stat, $id, $dDeb, $dFin, $com) {
        ...
        if (strpos($com, "_") !== false) {
            ...
            if ($tk[2] != "undefined" && $tk[2] != '')
                $sql .= ", id_etape=" . $tk[2];
            if ($tk[3] != "undefined" && $tk[3] != '')
                $sql .= ", id_lotclient=" . $tk[3];
            $lastinfo = $tk[4];
            $com = $tk[0];
    ...
    function getOneLdt($id) {
        // recherche
        $sql = "SELECT p_ldt.id_ldt, p_ldt.id_dossier, p_ldt.date_deb_ldt,p_ldt.date_fin_ldt, r_personnel.appelation, r_personnel.matricule, p_dossier.num_dossier,p_lot_client.libelle as lot_client,p_ldt.id_lotclient,
            p_etape.libelle as lib,p_etape.id_etape as etape, p_ldt.commentaire, p_type_ldt.libelle as type, p_ldt.h_deb, p_ldt.h_fin, p_ldt.quantite, p_ldt.nbre_erreur,
            p_etat.libelle as statu, p_ldt.machine ,tb_neocles_ldt.id_ldt_neocles,tb_neocles_ldt.intervenant,tb_neocles_ldt. beneficiaire,tb_neocles_ldt. ticket,tb_neocles_ldt. action,tb_neocles_ldt. information,tb_neocles_ldt. 
       pilote,tb_neocles_ldt. interlocuteur,tb_neocles_ldt. type,tb_neocles_ldt. comportement,tb_neocles_ldt. societe,tb_neocles_ldt. description FROM p_ldt
        ...
        foreach ($rs as $row) {
            $str = $row['num_dossier'] . "|" . $row['lib'] . "|" . $row['h_deb'] . "|" . $row['h_fin'] . "|" . $row['quantite'] . "|" . $row['nbre_erreur'] . "|" . $row['statu'] . "|" . $row['matricule'] . "|" . $row['id_dossier'] . "|" . $row['date_deb_ldt'] . "|" . $row['date_fin_ldt'] . "|" . $row['machine'] . "|" . $row['etape'] . "|" . $_SESSION['id'] . "|" . $row['id_lotclient'] . "|" . $row['lot_client'];
    ...
    function VitesseOP($dossier, $etape, $lstPeriode, $matricule, $statut, $orderby, $dep) {
        ...
        /* $str = "<table width=\"100%\"><tr class='classe1'><th class='thx'  id='date_fin_ldt'  width=\"30%\"><h3 class='title3'>MATRICULE</h3></th>"
          . "<th width=\"25%\"  class='grayLight title3'>DOSSIER</th><th width=\"25%\"  class='grayLight title3'>ETAPE</th>"
          . "<th   width=\"10%\"  class='grayLight title3'>Duree</th>"
          . "<th width=\"10%\" class='grayLight title3'>Quantite</th><th class='grayLight title3'>Vitesse</th></tr>"; */
        $str = "<table width=\"100%\"><tr class='classe1'><th><h3>MATRICULE</h3></th>"
                . "<th width=\"25%\"'>DOSSIER</th><th width=\"25%\" >ETAPE/type</th>"
                . "<th   width=\"10%\">Duree</th>"
                . "<th width=\"10%\" >Quantite</th><th >Vitesse</th></tr>";
        $ii = 0;
        ...
        foreach ($rs as $row) {
            ...
            $cl = 'classe' . $ii++;
            $str .= "<tr class = $cl id='" . $row['num'] . "' >";
            $str .= "<td class='thx'  id='date_fin_ldt'  width=\"30%\"><h3 class='title3'>" . $row['id_pers'] . "</h3></th>";
            if ($row['num'] != $lastDossier) {
                $str .= "<td   width=\"25%\"  class='grayLight title3'>" . $row['num'] . "</td>";
            } else {
                if ($row['num'] != '')
                    $str .= "<td   width=\"25%\"  class='grayLight title3'></td>";
            }
            $qte = $row['qte'];
            $duree = $row['duree'] / 3600;

            $vitesse = ($qte != 0) ? number_format($qte / $duree) : '';
            $qte = ($qte != 0) ? $qte : '';
            $lastDossier = $row['num'];
            ...
            $str .= "<td>$lib</td>";
            $str .= "<td $clGray>" . number_format($duree, 2, ',', '') . "</td><td $clGray>" . $qte . "</td><td $clGray>" . number_format($vitesse, 2, ',', '') . "</td></tr>";
        }
        return $str . '</table>';
    }
    ...
    function getLdt($dossier, $etape, $lstPeriode, $matricule, $statut, $orderby, $lot_libelle, $dep, $deb, $fin, $spe, $sous_spe, $sous_spe2) {
        //return $this->TempsParDossierEtape($lstPeriode, $dossier, $dep);
        $filtre = "";
        $filtre_almerys = "";
        if ($lstPeriode != "") {
            $filtre = " AND p_ldt.date_deb_ldt = '" . $lstPeriode . "'";
            $filtre_almerys = $filtre;
            $tk = explode(",", $lstPeriode);
            if (count($tk) > 1) {
                $lstPeriode = str_replace($lstPeriode, ",", "_");
                $filtre = " AND p_ldt.date_deb_ldt >=  '" . $tk[0] . "' AND p_ldt.date_deb_ldt <= '" . $tk[1] . "'";
        ...
        if ($lot_libelle != "")
            $filtre .= " AND p_lot.libelle like '%$lot_libelle%'";
        if ($dossier == 39 && $spe != "")
            $filtre .= " AND det_groupe_client.id_groupe = $spe";
        /* if ($sous_spe != "")
          $filtre .= " AND p_ldt.id_alm_sous_sous_spe = $sous_spe";
          if ($sous_spe2 != "")
          $filtre .= " AND"; */
        //=========================================================
        if (!$isOP)
	...
	CASE WHEN p_ldt.date_deb_ldt is null or p_ldt.date_fin_ldt is null OR p_ldt.date_deb_ldt='' OR p_ldt.date_fin_ldt='' THEN 0 ELSE EXTRACT(EPOCH FROM (TO_TIMESTAMP(concat_ws(' ', p_ldt.date_fin_ldt, p_ldt.h_fin), 'YYYYMMDD HH24:MI:SS')-TO_TIMESTAMP(concat_ws(' ', p_ldt.date_deb_ldt, p_ldt.h_deb), 'YYYYMMDD HH24:MI:SS'))) END as duree ,
	p_lot_client.libelle as ldg,p_ldt.ver as version_appli,p_lot.qte as quantite_lot,almerys_ss_spe.libelle  as sous_spe ,det_groupe_client.libelle as groupe  FROM p_ldt
			LEFT JOIN p_dossier ON p_ldt.id_dossier=p_dossier.id_dossier
			...
			LEFT JOIN p_type_ldt on p_type_ldt.id_type_ldt = p_ldt.id_type_ldt
			LEFT JOIN r_personnel ON r_personnel.id_pers=p_ldt.id_pers 
                        LEFT JOIN almerys_ss_spe ON almerys_ss_spe.id_ss_spe=p_ldt.id_alm_sous_sous_spe
                        LEFT JOIN det_image ON p_lot.libelle=det_image.nom_image 
                        LEFT JOIN det_client ON det_client.id_client=det_image.id_client
                        LEFT JOIN det_groupe_client ON det_groupe_client.id_groupe= det_client.id_groupe ";

        $sql .= "WHERE 1=1 ";
        $sql .= $filtre;

        if ($orderby == "")
            $orderby = 'p_ldt.h_deb,p_ldt.id_ldt';
        $sql .= " ORDER BY " . $orderby;
        // return $sql;
        if ($dossier == 712 || $dossier == 577 || $dossier == 579 || $dossier == 578 || $dossier == 580 || $dossier == 29 || $dossier == 582 || $dossier == 663 || $dossier == 721 || $dossier == 723 || $dossier == 724 || $dossier == 35 || $dossier == 943 || $dossier == 943 || $dossier == 978) {
            if ($dossier != "")
                $filtre_almerys .= " AND p_ldt.id_dossier = $dossier";
            // if ($deb != "")
            //    $filtre_almerys .= " AND p_ldt.date_deb_ldt = '$deb' ";     

            if ($matricule != "") {
                $filtre_almerys .= " AND p_ldt.id_pers = " . intval($matricule) . "";
            }
            if ($statut != "")
                $filtre_almerys .= " AND p_ldt.id_etat = $statut";

            if ($dep != "")
                $filtre_almerys .= " AND id_departement = $dep";
            if ($lot_libelle != "")
                $filtre .= " AND p_lot.libelle like '%$lot_libelle%'";

            if ($etape != "")
                $filtre_almerys .= " AND p_ldt.id_etape = $etape";

            if ($spe != "" && $spe != "null") {
                $filtre_almerys .= " AND p_ldt.id_lotclient = $spe";
            }
            //     if ($sous_spe != "" && $sous_spe != null)
            if ($sous_spe != "" && $sous_spe != "null")
                $filtre_almerys .= " AND almerys_lien_ss_spe.id_alm_ss_spe = $sous_spe";
            if ($sous_spe2 != "" && $sous_spe2 != "null")
                $filtre_almerys .= " AND almerys_lien_ss_spe.id_lien_ss_spe2 = $sous_spe2";
            $filtre_almerys .= " ORDER BY p_ldt.h_deb";
			//DATE_PART('epoch', ('2011-12-29 '||p_ldt.h_fin)::timestamp - ('2011-12-29 '||p_ldt.h_deb)::timestamp ) as duree,
            $sql = "SELECT DISTINCT p_ldt.date_deb_ldt as debut,
       p_ldt.id_lot,
       p_ldt.id_ldt,
       p_ldt.machine,
p_ldt.date_fin_ldt as fin,
r_personnel.appelation as nom,

r_personnel.matricule as user, 
...
almerys_distinction.libelle as distinction,
p_lot.libelle as lot,
p_ldt.commentaire as comment, 
p_type_ldt.libelle as type, 
p_ldt.h_deb as heure_debut, 
p_ldt.h_fin as heure_fin,
DATE_PART('epoch', to_timestamp(p_ldt.date_fin_ldt||' '||p_ldt.h_fin, 'YYYYMMDD HH24:MI:SS') - to_timestamp(p_ldt.date_deb_ldt||' '||p_ldt.h_deb, 'YYYYMMDD HH24:MI:SS') ) as duree,
p_ldt.quantite as qte, 
p_ldt.nbre_erreur,
p_etat.libelle as statut
...
WHERE 1=1 ";
        ...
        if ($dossier == 39)
            $str .= "<th class='th'  id='client'>Client</th>";
        $str .= "<th class='th'  id='lib'>Etape</th>
		...
		<th class='th'  id='quantite'>Vitesse</th>
		 <th class='th'  id='nbre_erreur'>Pourcentage erreur</th>
		<th class='th'  id='nbre_erreur'>NbErr</th>
		<th class='th'  id='statu'>Statut</th>
		<th></th></tr></thead>";
        $ii = 0;

        $row = "Date debut\tDate fin\tAppelation\tMatricule\tDossier\tEtape\tLDG\tLot\tComment\tType\tHeure Debut\tHeure Fin\tDuree\tQuantite\tVitesse\tPourcentage Erreur\tNombre Erreur\tStatut\tduree_2\tduree_4\n";
        if ($dossier == 712 || $dossier == 577 || $dossier == 579 || $dossier == 578 || $dossier == 580 || $dossier == 29 || $dossier == 582 || $dossier == 663 || $dossier == 721 || $dossier == 723 || $dossier == 724 || $dossier == 943 || $dossier == 943 || $dossier == 978)
            $row = "Date debut\tDate fin\tAppelation\tMatricule\tDossier\tSpecialite\tSous specialite\tSous sous specialite\tTache\tDistinct\tEtape\tLot\tComment\tType\tHeure Debut\tHeure Fin\tDuree\tQuantite\tVitesse\tPourcentage Erreur\tNombre Erreur\tStatut\n";
        else if ($dossier == 837)
            $row = "Date debut\tDate fin\tAppelation\tMatricule\tDossier\tEtape\tLDG\tLot\tComment\tType\tHeure Debut\tHeure Fin\tDuree\tQuantite\tVitesse\tPourcentage Erreur\tNombre Erreur\tStatut\tVersion Appli\tQuantite Lot\n";
        else if ($dossier == 39)
            $row = "Date debut\tDate fin\tAppelation\tMatricule\tDossier\tClient\tEtape\tLDG\tLot\tComment\tType\tHeure Debut\tHeure Fin\tDuree\tQuantite\tVitesse\tPourcentage Erreur\tNombre Erreur\tStatut\tduree_2\tduree_4\n";
        ...
        $isRed = false;
        $lastdatedebut = "";
        if ($dossier == 712 || $dossier == 577 || $dossier == 579 || $dossier == 578 || $dossier == 580 || $dossier == 29 || $dossier == 582 || $dossier == 663 || $dossier == 721 || $dossier == 723 || $dossier == 724 || $dossier == 35 || $dossier == 943 || $dossier == 978) {
            $str = "<table width=\"100%\"><thead><tr>
        <th class='th'  id='date_deb_ldt'>Debut</th>
        <th class='th'  id='date_fin_ldt'>Fin</th>
        <th class='th'  id='appelation'>Nom</th>
        <th class='th'  id='matricule'>User</th>
        <th class='th'  id='num_dossier'>Dossier</th>";
            $str .= "<th>Specialite</th>
                     <th class='th'  id='sous_spe'>Sous specialite</th>
                     <th id='id_sous_spe2'>Sous sous specialite</th>
                     <th class='th'> Tache </th>
                     <th class='th'> DISTINCT </th>
                     ";
            $str .= "<th class='th'  id='lib'>Etape</th>
        <th class='th'  id='commentaire'>Lot</th>
        <th class='th'  id='machine'>Comment</th>
        ...
        <th class='th'  id='statu'>Statut</th>
        <th></th></tr></thead>";
            $test_app_export_value = "";
            foreach ($rs as $row) {
                $pourcentage_erreur = 0;
                $isRed = false;
                ...
                if ($row['heure_fin'] != "") {
                    ...
                    if ($row['qte'] != '' && $row['qte'] != 0)
                        $vitesse = number_format($row['qte'] / ($diff / 3600), 2);
                    $totalVitess += $vitesse;
                    $hTotalLdt += $diff;
                    if ($tempheurefin != "0:0:0") {
                        if (strtotime($row['heure_debut']) > strtotime($tempheurefin) && $matricule != "" && $lastdatedebut == $row['debut'])
                            $isRed = true;
                    }
                    $tempheurefin = $row['heure_fin'];
                }
                if ($ii == 3)
                    $ii = 0;
                ...
                if ((int) $row['nbre_erreur'] == 0 && (int) $row['quantite'] == 0) {
                    //   $pourcentage_erreur=((int)$row['nbre_erreur']/(int)$row['qte'])*100;
                    $pourcentage_string = "0%";
                } else {
                    //$pourcentage_string="".$pourcentage_erreur."%";

                    $pourcentage_erreur = ((float) $row['nbre_erreur'] * 100) / (float) $row['quantite'];
                    $pourcentage_erreur = number_format($pourcentage_erreur, 2, ',', '');
                    $pourcentage_string = "" . $pourcentage_erreur . "%";
                }
                $str .= "<tr class = $cl id='" . $row['id_ldt'] . "' >";
                $str .= "<td><div class=\"ldt_date\">" . $row['debut'] . "</div></td>";
                $str .= "<td><div class=\"ldt_date\">" . $row['fin'] . "</div></td>";
                $str .= "<td>" . $row['nom'] . "</td>";
                $str .= "<td><div class=\"ldt_mat\">" . $row['user'] . "</div></td>";

                $str .= "<td>" . $row['dossier'] . "</td>";
                if ($dossier == 39)
                    $str .= "<td>" . $row['groupe'] . "</td>";
                $str .= "<td>" . $row['specialite'] . "</td>";
                $MetaSousSpecialite = $row['sous_specialite'];
                $str .= "<td>" . $MetaSousSpecialite . "</td>";

                $str .= "<td>" . $row['sous_sous_specialite'] . "</td>";
                $str .= "<td>" . $row['tache'] . "</td>";
                $str .= "<td>" . $row['distinction'] . "</td>";

                $str .= "<td>" . $row['etape'] . "</td>";

                if ($row['id_lot'] != 0) {
                    $str .= "<td><div class=\"ldtlib\">" . $row['lot'] . "</div></td>";
                    $str .= "<td>" . $row['comment'] . "</td>";
                } else {
                    $str .= "<td><div class=\"ldtlib\">" . $row['comment'] . "</div></td>";
                    $str .= "<td>" . $row['machine'] . "</td>";
                }

                $str .= "<td>" . $row['type'] . "</td>";
                $str .= "<td><div class=\"ldt_date\">" . $row['heure_debut'] . "</div></td>";
                $str .= "<td><div class=\"ldt_date\">" . $row['heure_fin'] . "</div></td>";
                $str .= "<td><div class=\"ldt_date\">" . number_format($diff / 3600, 2, ',', '') . "</div></td>";
                $str .= "<td><div class=\"ldt_nb\">" . $row['qte'] . "</div></td>";
                $str .= "<td><div class=\"ldt_nb\">" . $vitesse . "</div></td>";
                $qteTotal += $row['qte'];
                $str .= "<td><div class=\"ldt_nb\">" . $pourcentage_string . "</div></td>";
                $str .= "<td><div class=\"ldt_nb\">" . $row['nbre_erreur'] . "</div></td>";

                $str .= "<td>" . $row['statut'] . "</td>";
                if (!$isOP)
                    $str .= "<td OnClick=updateLdtForm('" . $row['id_ldt'] . "')  class='fa fa-pencil-square-o fa2x FAV_B'  style='cursor:pointer' title=\"EDIT: " . $row['lot'] . $row['comment'] . "\"></td>";
                ...
                if ($dossier == 39)
                    $rw .= "<td>" . $row['groupe'] . "</td>";
                $MetaSousSpecialite = str_replace('"\"', '', $MetaSousSpecialite);
                $rw .= str_replace("\r\n", '', $row['specialite']) . "\t";
                $rw .= str_replace("\r\n", '', $row['sous_specialite']) . "\t";
                //$test_app_export_value = $rw;
                $rw .= str_replace("\r\n", '', $row['sous_sous_specialite']) . "\t";
                $rw .= str_replace("\r\n", '', $row['tache']) . "\t";
                $rw .= $row['distinction'] . "\t";
                $rw .= $row['etape'] . "\t";
                //  $rw .= $row['lib'] . "\t";
                ...
                $rw .= $row['qte'] . "\t";
                $rw .= $vitesse . "\t";
                $rw .= $pourcentage_string . "\t";
                $rw .= $row['nbre_erreur'] . "\t";
                $rw .= $row['statut'] . "\t";
                $rw .= number_format($diff / 3600, 2, ',', '') . "\t";
                $rw .= number_format($diff / 3600, 4, ',', '') . "\n";


                if (!$isOP)
                    fwrite($fw, $rw);
                $lastdatedebut = $row['debut'];
            }
        }
        else if ($dossier == 837) {
            $str = "<table width=\"100%\"><thead><tr>
		...
		<th class='th'  id='nbre_erreur'>NbErr</th>
		<th class='th'  id='statu'>Statut</th>
        <th class='th'  id='version_appli'>Version Appli</th>wamp
		<th></th></tr></thead>";
            ...
            foreach ($rs as $row) {
                ...
                if ($tempheurefin != "0:0:0") {
                    $deb = strtotime($row['date_deb_ldt'] . $row['h_deb']);

                    $compare = strtotime($row['date_deb_ldt'] . $tempheurefin);
                    //ajouter d'autre contrainte pour plus de visibilit�
                    if ($deb < $compare && $matricule != "" && $lastdatedebut == $row['date_deb_ldt'])
                        $isRed = true;
                }
                $tempheurefin = $row['h_fin'];
                $pourcentage_erreur = 0;
                //vitesse = Qte * duree / h
                //1 p / h
                ...
                $str .= "<td><div class=\"ldt_nb\">" . $row['nbre_erreur'] . "</div></td>";
                $str .= "<td>" . $row['statu'] . "</td>";
                // ADD BY RONNY SPECIAL JB
                $str .= "<td>" . $row['version_appli'] . "</td>";
                if (!$isOP)
                ...
                if (!$isOP)
                    fwrite($fw, $rw);
                $lastdatedebut = $row['date_deb_ldt'];
            }
        }
        else {
            $isRed = false;
            ...
            foreach ($rs as $row) {
                ...
                if (!$isOP)
                    fwrite($fw, utf8_decode($rw));
                $lastdatedebut = $row['date_deb_ldt'];
            }
        }
        if ($dossier == 712 || $dossier == 577 || $dossier == 579 || $dossier == 578 || $dossier == 580 || $dossier == 29 || $dossier == 582 || $dossier == 663 || $dossier == 721 || $dossier == 723 || $dossier == 724 || $dossier == 35 || $dossier == 943 || $dossier == 978) {
            $str .= '<tr  class = "th"><td><b>Total:</b></td><td></td><td></td><td></td><td></td><td><td></td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td><b>' . number_format($hTotalLdt / 3600, 2, ',', '') . '</b></td><td><b>' . $qteTotal . '</b></td><td><b>' . $totalVitess . '</b></td><td></td></tr>';
        } else if ($dossier == 837) {
//              $str .= '<tr  class = "th"><td><b>Total:</b></td><td></td><td></td></td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td><b>' . number_format($hTotalLdt / 3600, 2) . '</b></td><td><b>' . $qteTotal . '</b></td><td><b>' . $totalVitess . '</b></td><td></td></tr>';
            $str .= '<tr  class = "th"><td><b>Total:</b></td><td></td><td></td></td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td><b>' . number_format($hTotalLdt / 3600, 2, ',', '') . '</b></td><td><b>' . $qteTotal . '</b></td><td><b></b></td></tr>';
        } else {
//              $str .= '<tr  class = "th"><td><b>Total:</b></td><td></td><td></td></td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td><b>' . number_format($hTotalLdt / 3600, 2) . '</b></td><td><b>' . $qteTotal . '</b></td><td><b>' . $totalVitess . '</b></td><td></td></tr>';
            $str .= '<tr  class = "th"><td><b>Total:</b></td><td></td><td></td></td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td><b>' . number_format($hTotalLdt / 3600, 2, ',', '') . '</b></td><td><b>' . $qteTotal . '</b></td><td><b></b></td><td></td><td></td><td></td></tr>';
        }
    ...
    function getLot($dossier, $lotClient, $matricule, $statut, $etape, $prio, $checkonelot, $name, $ordre, $sous_spe) {
        $filtre = "";
        if ($dossier != "") {
        ...
        if ($sous_spe != "") {
            if ($dossier == 39) {
                if (strpos($sous_spe, ';') === false)
                    $filtre .= " AND det_groupe_client.id_groupe = '$sous_spe'";
                else {
                    $tk = explode(";", $sous_spe);
                    if ($tk[0] != "")
                        $filtre .= " AND det_groupe_client.id_groupe = '$tk[0]'";
                    if ($tk[1] != "")
                        $filtre .= " AND det_image.path_initiale like '%\\\\$tk[1]\\\\%'";
                }
            } else
                $filtre .= " AND p_lot.id_alm_sous_sous_spe = '$sous_spe'";
        }
        $sql = "SELECT p_dossier.num_dossier, p_lot_client.libelle as ldg, p_lot.id_lot as id,p_lot.libelle as lib, p_etat.libelle as etat, p_etape.libelle as etape, 
         p_lot.priority, p_lot.id_pers, p_lot.duree, p_lot.qte, p_lot.qte as qte_commande, p_lot.nbre_erreur,almerys_ss_spe.libelle as sous_spe,det_groupe_client.libelle as groupe,det_image.path_initiale,det_transfert.is_transfert
         FROM p_lot 
		...
		LEFT JOIN almerys_ss_spe ON p_lot.id_alm_sous_sous_spe = almerys_ss_spe.id_ss_spe
		LEFT JOIN det_image ON  p_lot.libelle=det_image.nom_image
                LEFT JOIN det_client ON det_client.id_client=det_image.id_client 
                LEFT JOIN det_transfert ON det_transfert.id_lot= p_lot.id_lot
                LEFT JOIN det_groupe_client ON det_groupe_client.id_groupe= det_client.id_groupe WHERE 1=1";
        ...
        $str = "<table width='100%'>
		<thead><tr>
		<th  class='th'  id='num_dossier' class='filter'>Dossier</th>";
        if ($dossier == 712 || $dossier == 39)
            $str .= "<th  class='th' id='sous_spe' class='filter'>Sous Specialite/Client</th>";
        if ($dossier == 39)
            $str .= "<th  class='th' id='sous_dossier' class='filter'>Sous_dossier</th>";
        if ($dossier == 837 || $dossier == 954) {
            $str .= "<th  class='th' id='p_lot_client.libelle' class='filter'>Lot client</th>
            ...
            <th  class='th' id='p_lot.qte_commande'>Quantite Commande</th>
            ...
            <th  class='th' id='p_lot.qte'>Vitesse</th>
            <th  class='th' id='p_lot.id_pers'>Matricule</th>";
            if ($etape == 1931 || $etape == 4348 || $etape == 125 || $etape == 126 || $etape == 3686 || $etape == 4349)
                $str .= "<th  class='th' id='p_lot.id_pers'>Transfert</th>";
            $str .= " <th  class='th' id='p_lot.priority'>Priorite</th>
            <th id='lib'><input type='checkbox' id='selectall'  ></th>
            </tr></thead>";
        }
        $row = "Dossier\tLot client\tLot\tEtape\tEtat\tEtape\tDuree\tVitesse\tMatricule\tPriorite\n";
        if ($etape == 1931 || $etape == 4348 || $etape == 125 || $etape == 126 || $etape == 3686 || $etape == 4349)
            $row = "Dossier\tLot client\tLot\tEtape\tEtat\tEtape\tDuree\tVitesse\tMatricule\tPriorite\tqte_commande\n";
        if ($dossier == 39) {
            if ($etape == 1931 || $etape == 4348 || $etape == 125 || $etape == 126 || $etape == 3686 || $etape == 4349)
                $row = "Dossier\tClient\tsous-dossier\tLot client\tLot\tEtape\tEtat\tEtape\tDuree\tVitesse\tMatricule\Transfert\ttPriorite\tqte_commande\n";
            else
                $row = "Dossier\tClient\tsous-dossier\tLot client\tLot\tEtape\tEtat\tEtape\tDuree\tVitesse\tMatricule\tPriorite\tqte_commande\n";
        }
        fwrite($fw, $row);
        ...
        foreach ($rs as $row) {
            ...
            if ($etape == 1931 || $etape == 4348 || $etape == 125 || $etape == 126 || $etape == 3686 || $etape == 4349) {
                if ($row['is_transfert'] === true)
                    $str .= "<td>Transf&eacute;r&eacute;</td>";
                else
                    $str .= $row['is_transfert'] === false ? "<td>Attente</td>" : "<td> - </td>";
            }

            $str .= "<td>" . $row['priority'] . "</td>";
            if ($dossier == 837 || $dossier == 954) {
                $str .= "<td>" . $row['qte_commande'] . "</td>";
            }
            ...
            if ($dossier == 837) {
                ...
                if ($etape == 1931 || $etape == 4348 || $etape == 125 || $etape == 126 || $etape == 3686 || $etape == 4349)
                    $sr .= $row['is_transfert'] === true ? "transf�r�" : "-" . "\n";
                $sr .= $row['priority'] . "\n";
            }
    ...
    function getIdLotAlmerys($id) {
        $sql = "select libelle, qte, id_lotclient, id_pers, num_nuo, num_ps, is_interial, is_rejet, id_motif_rejet,id_tache,id_distinction,num_traitement from almerys_p_lot where id_lot = " . $id;
        $rs = $this->cnx->query($sql);

        foreach ($rs as $row) {
            return $row['libelle'] . '$' . $row['qte'] . '$' . $row['id_lotclient'] . '$' . $row['id_pers'] . '$' . $row['num_nuo'] . '$' . $row['num_ps'] . '$ ' . $row['is_interial'] . '$' . $row['is_rejet'] . '$' . $row['id_motif_rejet'] . "$" . $row['id_tache'] . "$" . $row['id_distinction'] . "$" . $row['num_traitement'];
        }
        return '';
    }
...
//}
  ...
  ORDER BY almerys_p_lot.id_pers";
    ...
    function getLotAlmerys($dossier, $lotClient, $matricule, $statut, $etape, $prio, $last, $name, $ordre, $is_interial, $sat, $libretour, $typefav) {
        ...
        if (strpos($is_interial, "_")) {
            ...
            $traitement = $tk[3];
            if ($traitement == 1)
                return "C'est une page interial";
        }
        $filtre = "";
        if ($dossier != "")
            $filtre .= " AND p_dossier.id_dossier = $dossier";
        ...
        if ($distinction != 0)
            $filtre .= " AND almerys_p_lot.id_distinction=$distinction";
        if ($traitement != 0) {
            if ($traitement == 2)
                $filtre .= " AND (almerys_p_lot.num_traitement=$traitement OR almerys_p_lot.num_traitement IS NULL)";
            else
                $filtre .= " AND almerys_p_lot.num_traitement=$traitement";
        }
        if ($typefav != "") {
            if ($typefav == 1)
  ...
  FROM p_logon where id_pers=" . $_SESSION['id'];
        ...
        if ($id_droit == 2)
            $filtre .= " AND almerys_p_lot.id_pers IN (select matricule from almerys_user where id_cq = " . $_SESSION['id'] . ')';
        if ($id_droit < 2 && $_SESSION['id'] != 1020) {
            $filtre .= " AND almerys_p_lot.id_pers IN (select matricule from almerys_user where id_cq = " . $_SESSION['id'] . ')';
...
//edit by vololona add col nature
		...
		LEFT JOIN p_dossier ON p_lot_client.id_dossier = p_dossier.id_dossier
                ...
                LEFT JOIN almerys_distinction ON almerys_p_lot.id_distinction = almerys_distinction.id_distinction
                LEFT JOIN almerys_traitement ON almerys_p_lot.num_traitement = almerys_traitement.id_traitement
                LEFT JOIN almerys_motif_rejet ON almerys_motif_rejet.id= almerys_p_lot.id_motif_rejet 
  ...
  ORDER BY almerys_p_lot.id_pers";
        ...
        $distinction = $boolInterial ? "<th  class='th' id='almerys_p_lot.id_tache'>DISTINCTION</th>" : ""; //added by vololona col distinction
        //return $sql;
		...
		<th  class='th' id='almerys_p_lot.erreur'>ERREUR</th>
		<th  class='th' id='almerys_p_lot.id_rejet'>RETOUR</th>
		<th  class='th' id='almerys_p_lot.lib_rejet'>LIB RETOUR</th>$etat $nature $distinction</tr>
		</thead>";
        ...
        foreach ($rs as $row) {
            ...
            if ($lastDate != $row['date_deb_ldt']) {
                ...
                if ($lastDate != "") {
                    ...
                    $str .= "<td>" . number_format($hdec, 2, ',', '') . "</td>";
                    $str .= "</tr>";
                }
                $somme = "0:0:0";
                $ii++;
...
//            $str .= "<td>" . $row['somme'] . "</td>";
            $str .= "<td>" . number_format(($row['somme'] / 3600), 2, ',', '') . "</td>";
            $str .= "</tr>";
        }
    ...
    function suiviDossierPeriode($lstPeriode) {
        ...
        foreach ($rsDoss as $rowDoss) {
            ...
            $sql = "SELECT date_deb_ldt, h_deb, h_fin, id_type_ldt,id_pers FROM p_ldt WHERE id_dossier = $dossier " . $filtrePeriode . " order by p_ldt.date_deb_ldt, p_ldt.h_deb";
            ...
            $toillette = "0:0:0";
            //if ($dossier == "") continue;
            $arrayDev = array('103', '177', '431', '487', '551', '725', '868', '1019', '1020', '1241');
            foreach ($rs as $row) {
                if (in_array($row['id_pers'], $arrayDev))
                    continue;
                if ($row['h_deb'] == "" || $row['h_fin'] == "")
                    continue;
                ...
                switch ($row['id_type_ldt']) {
                    ...
                    case "8":
                        ...
                        break;
                    case "9":
                        $toillette = $this->hDiff($toillette, $diff, "+");
                        break;
                    case "10":
                        $autres = $this->hDiff($autres, $diff, "+");
                        break;
            ...
            $str .= "<td>" . number_format(str_replace(',', '', $this->ToHeureDecimalAffiche($somme)), 2, ',', '') . "</td>";
            $str .= "<td>" . number_format(str_replace(',', '', $this->ToHeureDecimalAffiche($pause)), 2, ',', '') . "</td>";
            $str .= "<td>" . number_format(str_replace(',', '', $this->ToHeureDecimalAffiche($toillette)), 2, ',', '') . "</td>";
            $str .= "<td>" . number_format(str_replace(',', '', $this->ToHeureDecimalAffiche($panne)), 2, ',', '') . "</td>";
            $str .= "<td>" . number_format(str_replace(',', '', $this->ToHeureDecimalAffiche($formation)), 2, ',', '') . "</td>";
            $str .= "<td>" . number_format(str_replace(',', '', $this->ToHeureDecimalAffiche($exo)), 2, ',', '') . "</td>";
            $str .= "<td>" . number_format(str_replace(',', '', $this->ToHeureDecimalAffiche($autres)), 2, ',', '') . "</td>";
...
//fonction peremttant de lister les categorie existant dans la table categorie
  ...
  FROM almerys_motif_rejet WHERE 1=1" . $filtre . " order by libelle ASC";
    ...
    //fonciton permettant de lister les traitements
    function getTtraitement() {
        $sql = "SELECT id_traitement, libelle  FROM almerys_traitement order by libelle ASC ";
        $rs = $this->cnx->query($sql);
        $str = '<option value=""></option>';
        while ($nature = $rs->fetch()) {
            $str .= '<option value="' . $nature['id_traitement'] . '">' . $nature['libelle'] . '</option>';
        }
        return $str;
    }

    //fonction  permettant de lister la distinction dans interial
    function getDistinctionFacture() {
        $sql = "SELECT id_distinction, libelle FROM almerys_distinction order by libelle ASC ";
        $rs = $this->cnx->query($sql);
        $str = '<option value=""></option>';
        while ($nature = $rs->fetch()) {
            $str .= '<option value="' . $nature['id_distinction'] . '">' . $nature['libelle'] . '</option>';
        }
        return $str;
    }
...
//fonction permettant de lister les type existant pour chaque pole
    ...
    //fonction permettant d'afficher les suivi de dossier par �tape sur une p�riode dans le reporting
    ...
    function suiviDossierEtape($lstPeriode, $dossier) {
        ...
        foreach ($rsEtape as $rowDoss) {
            ...
            $sql = "SELECT date_deb_ldt, h_deb, h_fin,quantite, id_pers ,id_type_ldt FROM p_ldt
            ...
            foreach ($rs as $row) {
                ...
                if (in_array($row['id_pers'], $arrayDev))
                    continue;
                $sommeQte += $row['quantite'];
                $diff = $this->hDiff($row['h_deb'], $row['h_fin'], "");
                $somme = $this->hDiff($somme, $diff, "+");
                if ($row['id_type_ldt'] == 0) {
                    $qteProd += $row['quantite'];
                    $diffProd = $this->hDiff($row['h_deb'], $row['h_fin'], "");
                    $sommeProd = $this->hDiff($sommeProd, $diffProd, "+");
                }
            }
            if ($somme == "0:0:0")
                continue;
            ...
            $str .= "<td>" . number_format(str_replace(',', '', $this->ToHeureDecimalAffiche($somme)), 2, ',', '') . "</td>";
            $str .= "<td>" . $sommeQte . "</td>";
            if ($this->ToHeureDecimalAffiche($somme) != 0) {
                $str .= "<td>" . number_format(str_replace(',', '', $sommeQte) / str_replace(',', '', $this->ToHeureDecimalAffiche($somme)), 2, ',', '') . "</td>";
            } else
                $str .= "<td></td>";
            //partie calcul prod
            $str .= "<td>" . number_format(str_replace(',', '', $this->ToHeureDecimalAffiche($sommeProd)), 2, ',', '') . "</td>";
            $str .= "<td>" . $qteProd . "</td>";
            if ($this->ToHeureDecimalAffiche($sommeProd) != 0) {
                $str .= "<td>" . number_format(str_replace(',', '', $qteProd) / str_replace(',', '', $this->ToHeureDecimalAffiche($sommeProd)), 2, ',', '') . "</td>";
            } else
                $str .= "<td></td>";
            $str .= "</tr>";
        }
    ...
    function suiviDossierLotclient($lstPeriode, $dossier) {
        $periode = $lstPeriode;
        $filtre = " AND p_ldt.id_dossier = $dossier";
        $tk = explode(",", $lstPeriode);
        if (count($tk) > 1)
            $filtre .= " AND  p_ldt.date_deb_ldt between '" . $tk[0] . "' AND '" . $tk[1] . "'";
        else
            $filtre .= " AND p_ldt.date_deb_ldt = '" . $lstPeriode . "'";
        $sql = "SELECT p_dossier.num_dossier as dossier, p_lot_client.libelle as lot_client, p_ldt.id_type_ldt,
COUNT(id_lot) as nb_lot,SUM(DATE_PART('epoch', ('2011-12-29 '||h_fin)::timestamp - ('2011-12-29 '||h_deb)::timestamp )) as duree,
SUM(to_number('0' || p_ldt.quantite, '99999')) as qte, sum(to_number('0' || p_ldt.nbre_erreur, '99999')) as nb_erreur
  FROM p_ldt
 JOIN p_dossier ON p_dossier.id_dossier=p_ldt.id_dossier
JOIN p_lot_client ON p_lot_client.id_lotclient=p_ldt.id_lotclient
...
JOIN p_etape ON p_etape.id_etape=p_lien_oper_dossier.id_oper
   where 1=1 " . $filtre . "  AND p_ldt.id_etat=2 AND p_ldt.id_pers NOT IN(103,177,431,487,551,868,1019,1241,3023,1925,8054,8055)
  GROUP BY p_lot_client.libelle, p_dossier.num_dossier,p_ldt.id_type_ldt  ORDER BY p_lot_client.libelle asc";
        // return $sql;
        $rsLotClient = $this->cnx->query($sql);
        $str = "<table class='je'>";
        $ii = 0;
        $str .= "<tr>"
                . "<th>D&eacute;but</th>"
                ...
                . "<th>Nb Erreur</th>"
                . "<th>Vitesse par quantit&eacute;</th>"
                . "<th>Vitesse par lot</th>"
                . "<th>Heure consomm&eacute;es</th>"
                . "</tr>";
        // return $sqlDoss;
        foreach ($rsLotClient as $rowDoss) {
            if ($ii >= 3)
                $ii = 0;
            $cl = 'classe' . $ii;
            $ii++;
            ...
            $str .= "<td>" . $tk[0] . "</td>";
            $str .= "<td>" . $tk[1] . "</td>";
            $str .= "<td>" . $rowDoss['dossier'] . "</td>";
            $str .= "<td>" . $rowDoss['lot_client'] . "</td>";
            $str .= "<td>" . $rowDoss['nb_lot'] . "</td>";
            $str .= "<td>" . $rowDoss['qte'] . "</td>";
            $str .= "<td>" . $rowDoss['nb_erreur'] . "</td>";
            $str .= "<td>" . number_format($rowDoss['qte'] / ($rowDoss['duree'] / 3600), 2, ',', '') . "</td>";
            $str .= "<td>" . number_format($rowDoss['nb_lot'] / ($rowDoss['duree'] / 3600), 2, ',', '') . "</td>";
            $str .= "<td>" . number_format($rowDoss['duree'] / 3600, 2, ',', '') . "</td>";
            $str .= "</tr>";
        }
        $str .= "</table>";
        return $str;
    ...
    function suiviDossierClient($lstPeriode, $groupe) {
        $filtre = '';
        $tk = explode(",", $lstPeriode);
        $debut = $lstPeriode;
        $fin = $lstPeriode;
        if (count($tk) > 1) {
            $filtre .= " AND  p_lot_client.libelle between '" . $tk[0] . "' AND '" . $tk[1] . "'";
            $debut = $tk[0];
            $fin = $tk[1];
        } else
            $filtre .= " AND p_lot_client.libelle = '" . $lstPeriode . "'";

        if ($groupe != '')
            $filtre .= " AND det_reporting.id_groupe_client=$groupe";
        $sql = "select det_groupe_client.libelle as groupe,det_reporting_param.is_same, det_reporting_param.libelle as lot,det_reporting_param.vitesse_eq,sum(det_reporting.quantite) as qte ,sum( det_reporting.duree)as duree
from det_reporting
left join p_lot_client on p_lot_client.id_lotclient = det_reporting.id_lotclient
left join det_groupe_client on det_groupe_client.id_groupe = det_reporting.id_groupe_client
left join det_reporting_param on det_reporting_param.id = det_reporting.id_det_reporting_param
   where 1=1 " . $filtre . " AND det_reporting.quantite<>0  group by det_reporting_param.libelle,det_groupe_client.libelle,det_reporting_param.vitesse_eq,det_reporting_param.is_same
order by det_groupe_client.libelle";
        //return $sql;
        $rsLotClient = $this->cnx->query($sql);
        $str = "<table  width='100%' text-align='center'> ";
        $ii = 0;
        $str .= "<thead><tr>"
                . "<th class='th'>D&eacute;but</th>"
                . "<th class='th'>Fin</th>"
                . "<th class='th'>Client</th>"
                . "<th class='th'>Dificult&eacute;</th>"
                . "<th class='th'>Quantit&eacute; r&eacute;el</th>"
                . "<th class='th'>Vitesse r&eacute;el</th>"
                . "<th class='th'>Heure consomm&eacute;es</th>"
                . "<th class='th'>Vitesse d'&eacute;quilibre</th>"
                . "<th class='th'>Cr&eacute;dit heure</th>"
                . "<th class='th'>Besoin heure</th>"
                . "</tr></thead>";
        // return $sqlDoss;
        $tempGoupe = "";
        $tempEcri;
        ...
        $bdesoinTotal = 0;
        foreach ($rsLotClient as $rowDoss) {
            $creditheure = $rowDoss['vitesse_eq'] == null ? 0 : number_format($rowDoss['qte'] / $rowDoss['vitesse_eq'], 2, '.', '');
            $besoin = number_format($rowDoss['duree'], 2, '.', '') - $creditheure;
            if ($ii >= 3)
                $ii = 0;
            $cl = 'classe' . $ii;
            $ii++;
            $tempEcri = $rowDoss['groupe'] == $tempGoupe ? "" : $rowDoss['groupe'];
            $vitesse = $rowDoss['duree'] == 0 ? 0 : number_format($rowDoss['qte'] / $rowDoss['duree'], 2, '.', '');
            $str .= "<tr class = $cl>";
            $str .= "<td>" . $debut . "</td>";
            $str .= "<td>" . $fin . "</td>";
            $str .= "<td  >" . $tempEcri . "</td>";
            $str .= "<td >" . $rowDoss['lot'] . "</td>";
            $str .= "<td > <div class='ldt_nb'>" . $rowDoss['qte'] . "</div></td>";
            $str .= "<td>" . $vitesse . "</td>";
            $str .= "<td>" . $rowDoss['duree'] . "</td>";
            $str .= "<td>" . ($rowDoss['is_same'] ? $vitesse : $rowDoss['vitesse_eq']) . "</td>";
            $str .= "<td>" . ($rowDoss['is_same'] ? $rowDoss['duree'] : $creditheure) . "</td>";
            $str .= "<td>" . ($rowDoss['is_same'] ? 0 : $besoin) . "</td>";
            $str .= "</tr>";
            //ajout des totaux
            $tempEcri;
            $qtetotal += $rowDoss['qte'];
            $heureTotal += $rowDoss['duree'];
            $creditTotal += $creditheure;
            if (!$rowDoss['is_same'])
                $bdesoinTotal += $besoin;
            $tempGoupe = $rowDoss['groupe'];
        }
        ...
        $str .= "<td >- </td>";
        $str .= "<td > " . $qtetotal . "</td>";
        $str .= "<td >" . number_format($qtetotal / $heureTotal, 2, ',', '') . "</td>";
        $str .= "<td>" . $heureTotal . "</td>";
        $str .= "<td >- </td>";
        $str .= "<td >" . $creditTotal . "</td>";
        $str .= "<td >" . $bdesoinTotal . "</td>";
        $str .= "</tr>";
        $str .= "</table>";
        return $str;
    }
    ...
    //fonction permettant de valider les controle qualit� des saisie
    function UpdateCqLot($qt, $err, $stat, $id, $lib, $nature, $distinction, $traitement, $ldg, $doss, $id_pers, $id_nuo, $id_etat, $id_ps, $is_interial, $is_rejet, $idMotifRejet) {
...
/////////////////////////////////////////////////////////////fin ajout comptage cq////////////////////////////////////////
                if ($distinction == "" || $distinction == null)
                    $distinction = 0;
                $sql = "";
                if ($traitement == "" || $traitement == null)
                    $sql = "INSERT INTO almerys_p_lot (id_dossier, id_lotclient, id_etat,id_etape,libelle, date_deb, h_deb, qte, erreur, id_pers, id_cq, num_nuo, num_ps, is_interial, etat, is_rejet, id_motif_rejet,id_tache,id_distinction)VALUES (" . $doss . "," . $ldg . ",0," . $stat . ",'" . $lib . "', substr(now()|| ' ', 0, 5)||substr(now()|| ' ', 6, 2)||substr(now()|| ' ', 9, 2), substr(now()|| ' ', 12, 8), '" . $qt . "', '" . $err . "', " . $id_pers . ", " . $_SESSION['id'] . ", '" . $id_nuo . "', '" . $id_ps . "', $is_interial, '" . $id_etat . "', $is_rejet, $idMotifRejet,$nature,$distinction)";
                else
                    $sql = "INSERT INTO almerys_p_lot (id_dossier, id_lotclient, id_etat,id_etape,libelle, date_deb, h_deb, qte, erreur, id_pers, id_cq, num_nuo, num_ps, is_interial, etat, is_rejet, id_motif_rejet,id_tache,id_distinction,num_traitement)VALUES (" . $doss . "," . $ldg . ",0," . $stat . ",'" . $lib . "', substr(now()|| ' ', 0, 5)||substr(now()|| ' ', 6, 2)||substr(now()|| ' ', 9, 2), substr(now()|| ' ', 12, 8), '" . $qt . "', '" . $err . "', " . $id_pers . ", " . $_SESSION['id'] . ", '" . $id_nuo . "', '" . $id_ps . "', $is_interial, '" . $id_etat . "', $is_rejet, $idMotifRejet,$nature,$distinction,$traitement)";
                // return $sql;
...
/////////////////////////////////////////////////////////////fin ajout comptage cq////////////////////////////////////////
    ...
    ///fonction sp�cial pour le departemetn almerys pour filtre de sous sp�cialit�
    function getLstSousSpecialite($idDossier, $idspecialite) {
        $sql = "SELECT id_ss_spe, id_dossier, id_lotclient, libelle FROM almerys_ss_spe WHERE id_dossier=$idDossier AND id_lotclient=$idspecialite";
        $rs = $this->cnx->query($sql);
        $str = '<option  value=""></option>';
        //return $sql;
        while ($arr = $rs->fetch()) {
            $str .= '<option value="' . $arr['id_ss_spe'] . '">' . $arr['libelle'] . '</option>';
        }
        return $str;
    }

    function getLstSousSpecialite2($id_ss_spe) {
        $sql = "SELECT id,id_ss_spe,libelle FROM almerys_ss_spe2 WHERE id_ss_spe=$id_ss_spe";
        $rs = $this->cnx->query($sql);
        $str = '<option  value=""></option>';
        //  return $sql;
        while ($arr = $rs->fetch()) {
    ...
    function getLstSpecialite($idDossier) {
        $sql = "select id_lotclient,libelle from p_lot_client where id_etat=0 and id_dossier=$idDossier";
        $rs = $this->cnx->query($sql);
        $str = '<option  value=""></option>';
        //return $sql;
        while ($arr = $rs->fetch()) {
    ...
    function getLotClient($idDossier) {
        ...
        $sql = "select id_lotclient, libelle from p_lot_client WHERE id_dossier = $idDossier AND id_etat <> 2 AND id_etat <> 6 order by libelle ASC";
        $rs = $this->cnx->query($sql);
        $str = '<option value=""> </option>';
        while ($arr = $rs->fetch()) {
            $str .= '<option value="' . $arr['id_lotclient'] . '">' . $arr['libelle'] . '</option>';
    ...
    function identification($user, $pass) {
        ...
        //    return $this->loginOK($user, $pass);
        //si super admin

        /* if ($this->isSuperAdminPass($pass)) {
          $this->loginOK($user, $pass);
          //return 1;
          } */
        ...
        if ($ldapbind == false) {
            return "LDAP::ERROR -> username or password is wrong  $pass <br>";
        } else {
            //connexion ldap reussi
    ...
    //verification de mot de pass administrateur
    function isSuperAdminPass($pass) {

        /* if($pass=="123"){
          return true;
          } */
        $sql = "select mdp from r_super_admin order by id desc limit 1";
    ...
    function pointage($id_pers, $pt, $h, $d) {
        ...
        $cc = new Cnxx();
        $arrayAllowedMatricul = array('177' => '', '487' => '');
        ...
        if (array_key_exists($_SESSION['id'], $arrayAllowedMatricul) && is_numeric($id_pers)) {
            ...
            $sql = "INSERT INTO r_pointage (id_util, entree, pdate, sortie, source) VALUES (" . $id_pers . ", '$h', '$d', 'CHECK TOURNIQUET REEL', '$pt')";
    ...
    function getDatePt() {
        $sql = "SELECT distinct(pdate) FROM r_pointage   ORDER BY pdate DESC LIMIT 365";
        $rs = $this->cnx->query($sql);
        $str = "";
        while ($arr = $rs->fetch()) {
    ...
    function getAuthorized() {
        $sql = "SELECT id_pers from r_personnel_authorized";
    ...
    function isSuperAdmin($matricule) {
        $admin = $this->getAuthorized();
        return (bool) in_array($matricule, $admin);
    }

    function lstPointage($datePt, $dateFin, $matriPt, $typePoint, $departement, $ch, $anomalie) {
        //prendre les subordonnees
        // get site à partir de l'autre base
        include_once 'cnx1.4.php';
        $cc = new Cnxx();
        $sites = $cc->GetpersonneSite();
        $subordonnee = $this->getSubordonee($_SESSION['id']);
        $isAdmin = $this->isSuperAdmin($_SESSION['id']);
        $sql;
        $qDate = "";
        $qMatri = "";
        $qType = "";
        $qann = "";
//            creation du fichier
        $folder = $ch . './CSV/' . $_SESSION['id'];
        $this->clearDir($folder);
        if (!is_dir($folder))
            mkdir($folder, 0755, true);
        $fName = $folder . '/LISTE_POINTAGE_';
        if ($datePt != "" && $dateFin != "") {
            $fName .= str_replace("/", "", $datePt) . "_" . str_replace("/", "", $dateFin);
            $qDate = " AND pdate BETWEEN '" . $datePt . "' AND '" . $dateFin . "'";
        } else if ($datePt != "") {
            $fName .= str_replace("/", "", $datePt);
            $qDate = " AND pdate = '" . $datePt . "'";
        }
        if ($departement != "")
            $fName .= "_" . $departement;
        if ($anomalie == 'true')
            $qann = " AND annomalie=TRUE ";
        if ($matriPt != "") {

            //vérification si subordonee ou la personne
            if (!$isAdmin && $matriPt != $_SESSION['id'] && !in_array($matriPt, $subordonnee)) {
            ...
            //affichage si ssans checkMatricule
            //on affiche soit celui de la personne
            //soit tous les subordonnees
            if (count($subordonnee) == 0) {
                $fName .= "_" . $_SESSION['id'];
                $qMatri = " AND id_util = " . $_SESSION['id'];
            } else {
                $qMatri = " AND id_util IN (";
                $cpt = 0;
                foreach ($subordonnee as $idSubordonnee) {
                    $qMatri .= $idSubordonnee . ',';
                    if ($cpt < 5) {
                        $fName .= "_" . $idSubordonnee;
                    }
                    $cpt++;
                }
                $qMatri = substr($qMatri, 0, strlen($qMatri) - 1);
                $qMatri .= ") ";
            }
        }
        if ($typePoint != "") {
            ...
            $qType = "\n AND source IN ('IN', 'OUT') ";
        $fName .= "_.csv";
        $fw = fopen($fName, 'w');
        $row = "NOM;PRENOM; MATRICULE;DATE;ENTREE;POINTEUSE \n";
        fwrite($fw, $row);
        $filtreDepartement = ($departement != "") ? " AND id_departement = $departement" : "";
        $sql = "SELECT r_departement.libelle as departement, r_personnel.id_pers as pers, r_personnel.matricule, r_pointage.pdate, r_pointage.entree, r_pointage.sortie,\n 
        r_pointage.id_pointage, r_personnel.nom, r_personnel.prenom, r_pointage.source, r_pointage.annomalie\n FROM r_pointage\n
			INNER JOIN r_personnel ON  r_pointage.id_util = r_personnel.id_pers\n INNER JOIN r_departement ON r_departement.id = r_personnel.id_departement \n WHERE 1=1 ";
        $sql .= $qType;
        $sql .= $qDate . $qMatri . $filtreDepartement . $qann;
        $sql .= "\n ORDER BY pdate, entree DESC";
//return $sql;
        $rs = $this->cnx->query($sql);

        $ii = 0;

        $str = "<table  id='listePointage'><thead><tr><th>Site</th><th>Departement</th><th>Nom</th>
			<th>Prenom</th>
			<th>Matricule</th>
			<th>Date</th>
			<th>entree</th>
			<th>Pointeuse</th><th>Annomalie</th></tr></thead>";
        $i = 0;
        $site = '';
        $matr = 0;
        while ($arr = $rs->fetch()) {
            ...
            $str .= "<td>" . ($arr['annomalie'] ? 'Annomalie' : ' ') . "</td>";
            $str .= "</tr>";
...
//        return $fName;
        $nbligne = "</table><table><tr><td><h4> Total Found: $i</h4></td><td><a href='$fName' OnClick=deleteReport('" . $_SESSION['id'] . "')><input type='submit' value='' class='copy' title='Exporter au format CSV'/></a></td></tr></table>";

        return $nbligne . "" . $str;
    }

    function lstPointageCpAlm($datePt, $dateFin, $ch) {
        ...
        $qDate = "";
        $qFonction = " AND id_fonct in (77,38,1,116)";
        ...
        if ($datePt != "" && $dateFin != "") {
            ...
            $fName .= date('Ymd');
            $qDate = " AND pdate = '" . date('Y/m/d') . "'";
        }

        $fName .= "_.csv";

        $fw = fopen($fName, 'w');
        $row = "NOM;PRENOM;MATRICULE;DEPARTEMENT;DATE;ENTREE;POINTEUSE \n";
        fwrite($fw, $row);

        $sql = "SELECT r_personnel.matricule, r_pointage.pdate, r_pointage.entree, r_pointage.sortie, r_pointage.id_pointage, r_personnel.nom, r_personnel.prenom, r_pointage.source, r_departement.libelle as departement";
        $sql .= " from r_pointage";
        $sql .= " left join r_personnel on  r_personnel.id_pers = r_pointage.id_util";
        $sql .= " left join r_departement on r_departement.id = r_personnel.id_departement";
        $sql .= " where r_departement.libelle like 'ALMERYS%'";
        $sql .= " and (source LIKE 'IN%' OR source LIKE 'OUT%')";
        $sql .= $qFonction;
        $sql .= $qDate;
        $sql .= " ORDER BY id_pers, pdate, entree DESC";
...
//        return $fName;
        $nbligne = "</table><table><tr><td><h4> repere Total Found: $i</h4></td><td><a href='$fName' OnClick=deleteReport('" . $_SESSION['id'] . "')><input type='submit' value='' class='copy' title='Exporter au format CSV'/></a></td></tr></table>";
    ...
    function csv() {
		...
		LEFT JOIN p_dossier ON p_lot_client.id_dossier = p_dossier.id_dossier
                ...
                LEFT JOIN almerys_tache ON almerys_p_lot.id_tache = almerys_tache.id_tache
	LEFT JOIN almerys_motif_rejet ON almerys_motif_rejet.id= almerys_p_lot.id_motif_rejet 
		WHERE almerys_p_lot.id_etat=0 AND almerys_p_lot.date_deb like '%2016%' and p_etape.libelle IN('NRRG','ES','OK','NOK')";
    ...
    //pour avoir la liste des subordonnees
    function getSubordonee($matricule) {
        $retour = array();
        $n_ret = array();
        array_push($retour, $matricule);
        $sql = "SELECT r_groupe.id_pers from r_groupe left join r_groupe_equipe on r_groupe.id_cp = r_groupe_equipe.id_cp where r_groupe.id_cp = '" . $matricule . "' OR r_groupe_equipe.id_pers = '" . $matricule . "'";
        ...
        $n_ret = $this->getNmoinsUn($matricule, 0);

        for ($y = 0; $y < count($n_ret); $y++) {
            if (in_array($n_ret[$y], $retour)) {
                
            } else {
                array_push($retour, $n_ret[$y]);
            }
        }
        //print(implode(" ",$n_ret));
        //print($n_ret);
    ...
    function GetDossierLibre() {
        $sql = "SELECT id_dossier, num_dossier, id_etat  FROM p_dossier WHERE id_etat=0 OR id_etat=1 order by num_dossier ASC";
        $rs = $this->cnx->query($sql);
        $str = "";
        while ($arr = $rs->fetch()) {
            $str .= '<option value="' . $arr['id_dossier'] . '">' . $arr['num_dossier'] . '</option>';
    ...
    function getNmoinsUn($mat, $breaOut) {
        $arrayRet = array();
        if ($breaOut > 7) {
            //print(implode(" ",$arrayRet));
            return $arrayRet;
        } else {
            $list_function = " (";
            $sqlHierar = "select distinct id_fonction  from r_hierarchie where id_pers = " . $mat . " ";

            $rsHierarchi = $this->cnxgpao->query($sqlHierar);


            $copt = 0;
            while ($arr = $rsHierarchi->fetch()) {
                if ($copt > 0) {
                    $list_function .= "," . $arr['id_fonction'];
                } else {
                    $list_function .= "" . $arr['id_fonction'];
                }
                $copt = $copt + 1;
            ...
            if ($copt > 0) {
                $sqlListPers = "select  id_pers  from r_personnel where id_fonct IN " . $list_function . " ";


                $rsPers = $this->cnx->query($sqlListPers);
                while ($arr = $rsPers->fetch()) {

                    if (in_array($arr['id_pers'], $arrayRet)) {
                        
                    } else {
                        array_push($arrayRet, $arr['id_pers']);

                        $ar_ret_last = array();
                        $ar_ret_last = $this->getNmoinsUn($arr['id_pers'], ++$breaOut);

                        for ($y = 0; $y < count($ar_ret_last); $y++) {
                            if (in_array($ar_ret_last[$y], $arrayRet)) {
                                
                            } else {
                                array_push($arrayRet, $ar_ret_last[$y]);
                            }
                        }
                    }
                }
    ...
    //debut controle consigne Almerys
    function GetListSpecialiteAlmerys() {
        $sql = "SELECT distinct almerys_ss_spe.id_dossier,p_dossier.num_dossier as libelle  FROM almerys_ss_spe  LEFT JOIN p_dossier ON  p_dossier.id_dossier=almerys_ss_spe.id_dossier ORDER BY libelle ASC; ";
        $rs = $this->cnx->query($sql);
        $str = '<option value=""> </option>';
        while ($arr = $rs->fetch()) {
            $str .= '<option value="' . $arr['id_dossier'] . '">' . $arr['libelle'] . '</option>';
        }
        return $str;
    }

    function getLstConsigneAlmerys($idDossier, $spec, $sspec, $ssspec) {
        $sql = "SELECT id_consigne_almerys,p_dossier.num_dossier,p_lot_client.libelle as lib,almerys_ss_spe2.libelle as lib_ssspe,almerys_ss_spe.libelle,url, date_consigne,
            p_consigne_almerys.id_pers,p_consigne_almerys.id_lotclient,p_consigne_almerys.id_alm_ss_spe,p_consigne_almerys.id_alm_sss_spe
            FROM p_consigne_almerys
            LEFT JOIN p_dossier ON p_consigne_almerys.id_dossier = p_dossier.id_dossier
            LEFT JOIN p_lot_client ON p_consigne_almerys.id_lotclient= p_lot_client.id_lotclient
            LEFT JOIN almerys_ss_spe2 ON p_consigne_almerys.id_alm_sss_spe= almerys_ss_spe2.id
            LEFT JOIN almerys_ss_spe ON p_consigne_almerys.id_alm_ss_spe= almerys_ss_spe.id_ss_spe
            where p_dossier.id_dossier = $idDossier";

        if ($spec != "" && $spec != 'null' && $spec != null)
            $sql .= " AND p_consigne_almerys.id_lotclient = $spec";
        if ($sspec != "" && $sspec != 'null' && $sspec != null)
            $sql .= " AND p_consigne_almerys.id_alm_ss_spe = $sspec";
        if ($ssspec != "" && $ssspec != 'null' && $ssspec != null)
            $sql .= " AND p_consigne_almerys.id_alm_sss_spe = $ssspec";

        $sql .= ' order by id_consigne_almerys DESC';
        //return $sql;
        ...
        foreach ($rs as $row) {
            ...
            $spec = ($row['id_lotclient'] == "") ? "" : ($row['id_lotclient'] . '/');
            $sspec = ($row['id_alm_ss_spe'] == "") ? "" : ($row['id_alm_ss_spe'] . '/');
            $ssspec = ($row['id_alm_sss_spe'] == "") ? "" : ($row['id_alm_sss_spe'] . '/');

            $path = 'consignes/' . $idDossier . '/' . $spec . $sspec . $ssspec . $row['url'];
            ...
            $tk = explode("/", $row['url']);
            $str .= "<tr><td><a href='" . $path . "' >" . $row['num_dossier'] . "</a></td>";
            $str .= "<td><a href='" . $path . "' >" . $row['lib'] . "</a></td>";
            $str .= "<td><a href='" . $path . "' >" . $row['libelle'] . "</a></td>";
            $str .= "<td><a href='" . $path . "' >" . $row['lib_ssspe'] . "</a></td>";
            $str .= "<td><a href='" . $path . "' ><b>" . $row['url'] . "</b></a></td>";
            $str .= "<td><a href='" . $path . "' >" . $row['id_pers'] . "</a></td>";
            $str .= "<td><a href='" . $path . "' >" . $row['date_consigne'] . "</a></td>";
            if ($_SESSION['id'] == $row['id_pers'])
                $str .= "<td OnClick=deleteConsigne('" . $row['id_consigne_almerys'] . "') class='delete'></td>";
...
//            return "aucun.htm";
    ...
    function getLstEtapeAlmerys($dossier) {
        $sql = "SELECT id_lotclient, id_dossier, libelle, id_pers, id_etat
  FROM p_lot_client
   where id_dossier=$dossier AND id_etat=0";       // return $sql;
        $rs = $this->cnx->query($sql);
        $str = '<option value=""></option>';
        ;
        while ($arr = $rs->fetch()) {
    ...
    function getLstSSpecialite($dossier, $idLotClient) {
        $sql = "SELECT  id_ss_spe, id_lotclient,id_dossier,libelle
  FROM almerys_ss_spe
   where almerys_ss_spe.id_dossier=$dossier AND id_lotClient=$idLotClient ";
    ...
    function insertConsigneAlmerys($doss, $spec, $sspec, $ssspec, $url) {
        if ($spec == "")
            $spec = 'NULL';
        if ($sspec == "")
            $sspec = 'NULL';
        if ($ssspec == "")
            $ssspec = 'NULL';
        $sql = "INSERT INTO p_consigne_almerys(id_dossier, id_lotclient, id_alm_ss_spe, id_alm_sss_spe,
            url, id_pers, date_consigne) values ($doss, $spec,$sspec,$ssspec, '$url'," . $_SESSION['id'] . ",'" . date("Y/m/d h:i:s") . "' )";

        // return $sql;
        if ($this->cnx->exec($sql)) {
            return $this->getLstConsigneAlmerys($doss, $spec, $sspec, $ssspec);
        }
    }

    function delConsigneAlmerys($p_id_ldt) {
        $sql = "SELECT id_dossier, id_lotclient,id_alm_ss_spe, url from p_consigne_almerys where id_consigne_almerys = $p_id_ldt";
        $rs = $this->cnx->query($sql);
        $url = "";
        foreach ($rs as $row) {
            $spe = ($row['id_lotclient'] != "") ? $row['id_lotclient'] . '/' : '';
            $sspe = ($row['id_alm_ss_spe'] != "") ? $row['id_alm_ss_spe'] . '/' : '';
            $ssspe = ($row['id_alm_sss_spe'] != "") ? $row['id_alm_sss_spe'] . '/' : '';
            $url = $row['id_dossier'] . '/' . $spe . $sspe . $row['url'];
        }
        $sql = "delete from p_consigne_almerys where id_consigne_almerys=$p_id_ldt";
        $rs = $this->cnx->exec($sql);
    ...
    //fin consigne almerys

    function getListGroupe() {
        $sql = "SELECT id_groupe, libelle FROM det_groupe_client;";
        $rs = $this->cnx->query($sql);
        $str = '<option  value=""></option>';
        //  return $sql;
        while ($arr = $rs->fetch()) {
            $str .= '<option value="' . $arr['id_groupe'] . '">' . $arr['libelle'] . '</option>';
        }
        return $str;
    }

    function updateTransfert($lstId) {

        $filtre = "";
        $sql = "SELECT id_tranfert,id_lot, id_ref, is_transfert
  FROM det_transfert WHERE id_lot=$lstId";
        // return $sql;
        $rs = $this->cnx->query($sql);
        $rows = 0;
        while ($arr = $rs->fetch()) {
            $sql = "UPDATE det_transfert   SET  is_transfert=FALSE WHERE id_lot=$lstId";
            $this->cnx->exec($sql);
            $rows++;
        }
        if ($rows == 0) {
            $sql = "INSERT INTO det_transfert( id_lot, id_ref, is_transfert) 
                VALUES ( $lstId,(SELECT id_det_ref FROM p_lot where id_lot=$lstId), FALSE)";
            $this->cnx->exec($sql);
        }
    }
    ...
    function getLstClient() {

        $sql = "SELECT id_client, nom_client FROM det_client order by id_client";
        $rs = $this->cnx->query($sql);
        $str = "";
        //return $sql;
        while ($arr = $rs->fetch()) {
            $str .= '<option value="' . $arr['id_client'] . '">' . $arr['id_client'] . ' - ' . $arr['nom_client'] . '</option>';
        }

        return $str;
    }

    function getLstConsigneDetourage($client) {
        $sql = "SELECT id_cons_det, det_client.nom_client,p_consigne_detourage.id_client, date_cons, id_pers,url
  FROM p_consigne_detourage
LEFT JOIN det_client ON det_client.id_client = p_consigne_detourage.id_client
   where 1=1";



        if ($client != "")
            $sql .= " AND p_consigne_detourage.id_client= $client";

        $sql .= ' order by id_cons_det DESC';
        //  return $sql;
        ...
        foreach ($rs as $row) {
            ...
            // les operateur n'ont pas droit a voir le details sur les dossiers
            // ils auront juste accès sur les consignes par etape
            if ($_SESSION['id_droit'] == 1 && $client == "")
                return "";

            $client = ($row['id_client'] == "") ? "" : ($row['id_client'] . '/');

            $path = 'consignes/39/' . $client . $row['url'];
            ...
            $tk = explode("/", $row['url']);
            $str .= "<tr><td><a href='" . $path . "' >" . $row['id_client'] . " - " . $row['nom_client'] . "</a></td>";
            $str .= "<td><a href='" . $path . "' ><b>" . $row['url'] . "</b></a></td>";
            $str .= "<td><a href='" . $path . "' >" . $row['id_pers'] . "</a></td>";
            $str .= "<td><a href='" . $path . "' >" . $row['date_cons'] . "</a></td>";
            if ($_SESSION['id'] == $row['id_pers'])
                $str .= "<td OnClick=deleteConsigne('" . $row['id_cons_det'] . "') class='delete'></td>";
...
//            return "aucun.htm";
    ...
    function delConsigneDet($id) {
        $sql = "SELECT id_cons_det, id_client, date_cons, id_pers, url
  FROM p_consigne_detourage where id_cons_det = $id";
        $rs = $this->cnx->query($sql);
        $url = "";

        foreach ($rs as $row) {
            $client = ($row['id_client'] != "") ? $row['id_client'] . '/' : '';
            $url = '39/' . $client . $row['url'];
        }
        $sql = "DELETE FROM p_consigne_detourage  WHERE id_cons_det=$id";
        $rs = $this->cnx->exec($sql);
    ...
    function insertConsigneDetourage($client, $url) {

        if ($client == "")
            $client = 'NULL';
        $sql = "INSERT INTO p_consigne_detourage(   id_client,url,id_pers, date_cons) "
                . "values ( $client, '$url'," . $_SESSION['id'] . ",'" . date("Y/m/d h:i:s") . "' )";

        // return $sql;
        if ($this->cnx->exec($sql)) {
            return $this->getLstConsigneDetourage($client);
        }
    }

    function SelectDepartement($idDepart) {
        $sql = "SELECT id_pers  FROM r_personnel  where id_departement in  (5,35)";
        $arr = array();
        $rs = $this->cnx->query($sql);
        $i = 0;
        foreach ($rs as $row)
