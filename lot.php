        ...
        <center>
            ...
            <div id="optionsLot">
                ...
                <div id="oContent">
                    ...
                    <input type="submit" id="tranfert"  value="Transfert" title="Transfert" class="btOption"/><br/><br/>
                        ...
                        <td>
                            <table>
                                <tr>
                                    ...
                                    <td id="sousspehide">
                                        <span>Sous specialite/Client</span><br/>
                                    ...
                                    <td>	
                                        <span>LOT  <b>  <input type="checkbox" id="CheckLotSimple"/></span><br/>
                                        <input type="text" id="lotName" />
                                    </td>
                                    
                                    <td id="sous_dossier">	
                                        <span>Sous-dossier</span><br/>
                                        <input type="text" id="sous_doss" />
                                    </td>
                                    <td>	
...
// DAT 2012 03 06
    ...
    function updateLot(etat, priorite, mat)
    {
        ...
        $("input:checked").each(
                function () {
                    if ($(this).attr("id") != 'selectall')
                            ...
                            {
                                nb++;
                                listeIDS[nb] = $(this).attr("id");
                                //   alert( nb+"======>"+listeIDS[nb]);
                            }
                    //  alert("nombre =>"+nb);
                }
        );
        ...
        {
            /*
             ****** SAVE CHANGE FROM STRING ARRAY SEPARATOR =====> ","
            ...
            //    alert("stop LENGHT===>"+listeIDS.length);
            for (i = 1; i < listeIDS.length; i++) {
                $.ajax({
                    type: "GET",
                    url: "php/link.php?action=updateLot&lstId=" + listeIDS[i] + "&etat=" + etat + "&prio=" + priorite + "&mat=" + mat,
                    success: function (msg) {
                        console.log("ok no error found!");
                         // alert(msg);
                    },
                    error: function (err) {
    ...
    function updateTransfert()
    {
        ...
        if (nb != 0)
        {
            for (i = 1; i < listeIDS.length; i++) {
                $.ajax({
                    type: "GET",
                    url: "php/link.php?action=updateTransfert&lstId=" + listeIDS[i],
                    success: function (msg) {
                        console.log("ok no error found!");
                        alert(msg);
                    },
    ...
    $("#tranfert").click(function () {
        var prio = $("#prio").val();
        updateTransfert("", prio, "");
    });
    ...
    {
        var CheckOneLot = $("#CheckLotSimple:checkbox:checked").length > 0;
        ...
        var checkonelot = CheckOneLot.toString();
        var name = $("#lotName").val();
        var sous_spe = $("#sous_specialite").val();
        var sous_dossier = $("#sous_doss").val();
        if (doss == "")
        {
            alert("choisissez un dossier!");
            return;
        }
        if(doss==39 && sous_dossier!==null)
            sous_spe += ";"+ sous_dossier;
        $("#ldtCorp").html('<div class="body" id="Etape"><img src = "img/ramiLoad.gif"/>Loading ...</div>');
        $.ajax({
            type: "GET",
            url: "php/link.php?action=getLstLot&doss=" + doss + "&ldg=" + ldg + "&mat=" + mat + "&stat=" + stat + "&etape=" + etape + "&prio=" + prio + "&checkonelot=" + checkonelot + "&name=" + name + "&ordre=" + ordre + "&sous_spe=" + sous_spe,
            success: function (msg) {
